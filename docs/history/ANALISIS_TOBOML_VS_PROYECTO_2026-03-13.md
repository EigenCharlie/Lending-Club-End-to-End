> **HISTORICAL** — Snapshot archivado. No usar como superficie operativa viva; para el estado actual usa `docs/DOCUMENTATION_MAP.md` y los artifacts canónicos.
...
- `reports/history/project_audit_snapshot.json` contiene metricas historicas que no corresponden al baseline oficial actual.
- `reports/history/INFORME_INTEGRAL_PROYECTO_RIESGO_CREDITO_ES.md` es util como contexto, pero varias cifras y afirmaciones quedaron viejas.
