...
Antes de entrar al detalle, conviene ubicar el resultado frente a la literatura más cercana. La prueba usa el lenguaje moderno de exchangeability, cuantiles conformales y split conformal sintetizado por [@angelopoulos2026_foundations]. [@bates2021rcps] y [@angelopoulos2025ltt] muestran cómo calibrar predictores para controlar riesgo distribution-free; [@angelopoulos2024risk] demuestra que la predicción conformal puede controlar la esperanza de cualquier pérdida monotónica acotada. Nuestro Teorema 1 hereda ese espíritu, pero lo instancia en una dirección distinta: la pérdida no es un error genérico de predicción, sino la **no cobertura ponderada por funded set** que aparece al mapear intervalos conformales a una restricción lineal de PD de portafolio.

| Trabajo | Objeto garantizado | Vinculo con decisión | Diferencia con este paper |
|---|---|---|---|
| [@angelopoulos2026_foundations] | fundamentos de exchangeability, cuantiles conformales y split conformal | base formal de la capa de incertidumbre | no conecta por si solo intervalos con una decision de portafolio |
...
| [@barber2021_limits_conditional] | limites de cobertura condicional distribution-free | frontera de claims por subgrupo/source | Paper Estrella reporta cobertura verificable y Mondrian, no validez condicional exacta individual |
...
| [@bao2025croms] | seleccion de modelos conformales por riesgo decisional | selector decision-aware sobre candidatos conformales | en este paper aparece solo como auditoria tipo CROMS sobre finalists; no como entrenamiento end-to-end |
...
#### Lectura como conformal risk control

El bound puede leerse como una instancia de *conformal risk control* con una
perdida monotónica acotada. En CRC, se escoge un parametro de conservadurismo
para que la perdida esperada de un nuevo punto quede bajo un nivel $\alpha$
[@angelopoulos2024risk]. En CRPTO, el parametro operativo es el nivel conformal
que ensancha $u_i(\alpha)$; la perdida relevante no es miscoverage marginal
cruda, sino la no cobertura ponderada por el funded set:

$$
L_{\alpha}(x_i,y_i;w_i)=w_i\,\mathbf{1}\{Y_i>u_i(\alpha)\}.
$$

Esta lectura es util para reviewers porque explica por que el teorema no es una
pieza aislada: el paper especializa la familia CRC/RCPS/LTT a una perdida
decisional que aparece dentro de una restriccion lineal de portafolio. Aun asi,
la version actual no recalibra $\alpha$ con un algoritmo CRC directo sobre la
perdida de decision; usa la garantia conformal y luego valida `V`,
`Gamma_CP` y `violation` sobre la policy congelada. Una extension natural seria
calibrar directamente un gate CRC/LTT sobre `V` o sobre
`max(0, sum_i w_iY_i - tau)` en un split prospectivo.
...
#### Checklist de lectura para el bound

Para evitar sobrelecturas, el bound se defiende con cuatro reglas:

| Pieza | Claim permitido | Lo que no autoriza |
|---|---|---|
| Exchangeability / split conformal | cobertura marginal del target observado bajo el split declarado | cobertura condicional exacta o garantía individual |
| Teorema 1 / Markov | control distribution-free de la violación ponderada vía $\mathbb{E}[V] \leq \alpha$ y $\mathbb{P}(V>t)\leq\alpha/t$ | una cota apretada para todo $t$ o una garantía post-selección más fuerte |
| Lemma 1 / Hoeffding-Bernstein | tightening condicional bajo independencia adicional de los indicadores $Z_i$ | reemplazar el resultado principal sin verificar dependencia |
| Cierre `276k` | validación exacta de la policy congelada y de la región robusta observada | prueba prospectiva de que cualquier policy futura pasará el mismo gate |

: Checklist reviewer-facing para leer el bound sin overclaim {#tbl-bound-claim-checklist}
...
La @fig-alpha-gamma-bound muestra la validación empírica del vínculo teórico en 8 niveles de $\alpha$. A medida que $\alpha$ decrece (mayor confianza), $\Gamma_{\text{CP}}$ crece monótonamente --- confirmando la @sec-prop-monotonicity --- y la cobertura empírica se mantiene por encima del target nominal en todos los niveles. Sin embargo, la lección final del proyecto es que esa validación, por sí sola, todavía no basta para la afirmación más exigente del paper: hace falta además que el portfolio promovido convierta ese presupuesto conformal en un funded set exacto y defendible.

![Validación del vínculo $\alpha \leftrightarrow \Gamma_{\text{CP}}$ sobre el champion económico evaluado en 276K préstamos OOT ($\tau = 0.175$, $\gamma = 0.45$). **Panel A**: presupuesto conformal $\Gamma_{\text{CP}}(\alpha)$ — monótonamente decreciente en $\alpha$, confirmando la @sec-prop-monotonicity. **Panel B**: tasa de violación empírica del constraint de PD ponderado vs. cota teórica $\alpha/\sqrt{\alpha}$ — la violación es cero en todos los niveles. **Panel C**: miscoverage ponderada $V$ vs. cota $\sqrt{\alpha}$ del @sec-thm-feasibility(c) — todos los niveles satisfacen $V \leq \sqrt{\alpha}$.](../../assets/figures/publication/estrella_fig_alpha_gamma_bound.png){#fig-alpha-gamma-bound}
...
La cota $\alpha/t$ del @sec-thm-feasibility es teóricamente correcta, pero para varios valores de $t$ resulta demasiado floja para ser la señal operativa principal. En la práctica, el cierre final del proyecto se ordena mejor con:
...
El @sec-thm-feasibility traduce la garantía estadística del intervalo conformal en una garantía operativa sobre el portafolio. La tabla siguiente recuerda la escala teórica general:
...
Por la descomposición del @sec-thm-feasibility, esto implica una cota condicional análoga para $\mathbb{P}(\sum_i w_iY_i>\tau+\mu_w+s\mid\mathcal{F}_{cal})$.
:::
...
El Lemma 1 **no reemplaza** el @sec-thm-feasibility. Markov sigue siendo la garantía distribution-free principal porque solo requiere control de $\mathbb{E}[V]$. Hoeffding/Bernstein son útiles para una versión journal más fuerte, pero dependen de independencia condicional de los indicadores de no cobertura, supuesto que split conformal no garantiza estrictamente porque todos los préstamos comparten la misma muestra de calibración.
:::

El apéndice de dependencia `docs/research/paper_estrella_conditional_tightening_appendix_2026-05-04.md` fija la regla editorial para no sobreprometer este resultado: Markov es el claim principal, mientras Hoeffding/Bernstein quedan como tightening condicional bajo independencia adicional o bajo una futura prueba de concentración con dependencia controlada.

La literatura reciente abre un frente adicional, pero todavia no cambia el
claim. [@angelopoulos2026_nonmonotonic_crc] extiende el control de riesgo hacia
perdidas no monotónicas y parámetros multidimensionales bajo condiciones de
estabilidad. Eso sería relevante si una version futura quisiera controlar de
forma conjunta retorno, `V`, CVaR, source governance y fairness proxy. Hoy
CRPTO conserva el claim monotónico simple: mayor conservadurismo ensancha
intervalos, aumenta `Gamma_CP` y reduce la exposicion a no cobertura.
...
::: {#cor-mondrian}
Bajo predicción conformal Mondrian con grupos $\{g_1, \dots, g_G\}$, la cota del @sec-thm-feasibility se refina a:
...
- **SPO+** entrena end-to-end para minimizar regret de decisión. En este proyecto, el rerun final PyEPO 1.3.7/Gurobi logra una reducción de regret del `48.51%` sobre el two-stage clásico (5 seeds, 1,000 observaciones pareadas, Wilcoxon $p = 3.80\times10^{-163}$). Su fortaleza es la eficiencia decisional.
...
### Posicionamiento en el marco universal de Powell (SDAM) {#sec-estrella-sdam}
...
La ventaja de la CFA es triple: es **determinista** (un LP resuelto una vez con HiGHS en milisegundos), **auditable** (el regulador puede inspeccionar la restricción $\sum w_i u_i(\alpha) \leq \tau$ directamente) y **matemáticamente fundamentada** (la garantía de cobertura conformal del @sec-thm-feasibility se hereda a la restricción de forma verificable).

::: {.callout-important}
## Sobre la naturaleza uniperiodo

La formulación actual es estrictamente uniperiodo ($T = 1$): se toma una decisión de asignación y se observa un único vector de defaults. Esto es el caso más simple del framework SDAM, donde la función de transición $S^M$ es trivial (calcula P\&L) y no hay secuencia de decisiones adaptativas. La riqueza secuencial del framework --- actualización de creencias $B_t$, evolución de recursos $R_t$, encadenamiento de decisiones --- queda latente y se activa en la extensión a producción (véase @sec-future-sdam-production).
:::

#### Interpretación de $\alpha$ como parámetro de sintonización de política

A diferencia de los parámetros de penalización en optimización robusta clásica --- que se calibran por convención o juicio experto --- el nivel conformal $\alpha$ tiene una semántica estadística directa: controla la probabilidad de no cobertura del intervalo predictivo. Esto significa que el parameter tuning de la CFA no es heurístico sino que está anclado a una garantía distribution-free sobre la información exógena $W_{t+1}$. En el lenguaje de Powell, $\alpha$ es un tunable parameter cuyo efecto sobre la contribución $C(S_t, x_t)$ puede cuantificarse formalmente a través del @sec-thm-feasibility y el @sec-cor-por.
