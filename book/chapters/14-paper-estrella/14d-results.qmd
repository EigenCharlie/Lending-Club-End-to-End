...
: Resumen predictivo del upstream canónico {#tbl-p1-results-core}
...
: Tests estadísticos de calibración (KS, Kuiper, Spiegelhalter). Ambos modelos rechazan a $n = 276{,}869$, pero el calibrado mejora entre 6× y 60,000× en p-value. {#tbl-p1-calibration-tests}
...
: Comparación de conjuntos de incertidumbre para optimización robusta {#tbl-p1-baseline}

La ventaja competitiva del enfoque conformal no es solo cobertura --- es la combinación de **cobertura verificable**, **anchura controlable vía $\alpha$** y **garantía distribution-free sin supuestos sobre el DGP**. El bootstrap requiere re-entrenamiento múltiple sin garantías finitas; BMA requiere especificación correcta del espacio de modelos; Bertsimas-Sim deja abierta la calibración de $\Gamma$. CRPTO hereda el $\Gamma$ directamente de la capa conformal (@eq-gamma-cp).
...
```{python}
#| label: tbl-p1-uncertainty-baselines-artifact
...
```{python}
#| label: tbl-p1-uncertainty-baselines-worst-grade
...
```{python}
#| label: tbl-p1-progression-reading
...
```{python}
#| label: tbl-p1-ab-bootstrap-guard
...
pd.DataFrame(
    [
        ...
        },
        {
            ...
            "Lectura": "retorno oficial reportable del Paper Estrella",
        },
    ]
)
```
...
El @sec-cor-por establece que $\text{PoR}(\alpha) \lesssim \Gamma_{\text{CP}}(\alpha) \cdot \overline{LGD} / \bar{r}$. Con los números del champion economic instanciamos:
...
donde $\bar{r}_{\text{funded}}$ es el retorno neto promedio del funded set. La cifra empírica observada es `price_of_robustness_pct = -10.56%` (`-$14,466` sobre un retorno realizado de `$170,464.54`). Esto significa que la policy paga ~10.6% de retorno por la garantía conformal exacta a $\alpha = 0.01$, dentro del orden de magnitud que el corollary predice. La cota teórica es **conservadora pero no vacua**: un revisor puede verificar que el precio empírico observado no excede el techo teórico derivado del presupuesto conformal.

El comparador theorem-tight (gamma=0.55) baja $\Gamma_{\text{CP}}$ a `0.159714` con un retorno menor (`$166,270`), instanciando exactamente la frontera del corollary: menor presupuesto conformal → menor precio de robustez → menor retorno. Las tres policies (economic, balanced, theorem-tight) ordenan limpiamente esa frontera empírica dentro de la región robusta.

::: {.column-page}
![Comparación de decision regret entre three-stage, SPO+ end-to-end y CRPTO (Conformal Robust). El rerun final PyEPO 1.3.7/Gurobi muestra que SPO+ logra 48.51% menos regret que two-stage por diseño; CRPTO paga un precio de auditabilidad mayor en regret a cambio de garantías formales de cobertura.](../../assets/figures/publication/estrella_fig9_spo_regret.png){#fig-p1-spo-regret fig-alt="Comparación de regret: two-stage vs SPO+ vs CRPTO para el Paper Estrella."}
:::

::: {.callout-note}
## Lectura correcta del regret

La @fig-p1-spo-regret no muestra que CRPTO "pierde" contra SPO+; muestra el **trade-off diseñado**: SPO+ optimiza regret directamente (`48.51%` de reducción en el rerun final), mientras CRPTO optimiza robustez bajo incertidumbre auditable. El regret más alto de CRPTO es el **precio de la auditabilidad** --- la policy es más conservadora porque protege contra realizaciones adversas con cobertura verificable (92.97%). Un regulador puede auditar la cobertura de CRPTO; no puede auditar si el regret de SPO+ se mantendrá bajo distributional shift.
:::
...
```{python}
#| label: tbl-p1-cqr-comparator-reading
...
```{python}
#| label: tbl-p1-alpha-sweep-artifact-reading
...
`alpha_sweep_pareto_both.parquet` no es una nueva selección de champion. Es un artefacto de sensibilidad que explica la geometría cobertura-ancho al mover $\alpha$. Las métricas oficiales del Paper Estrella siguen ancladas en `models/final_project_promotion.json::conformal_upstream.winner_metrics` y en la evaluación exacta `276k`. Esta separación evita mezclar familias: el sweep enseña la palanca; la promoción final decide la policy.
:::
...
### Evidencia P1 añadida para versión journal {#sec-estrella-p1-evidence}

La auditoría posterior al cierre del champion agregó una capa P1 reproducible que endurece el paper sin cambiar la policy oficial. Las tablas A3--A6 se regeneran desde `scripts/export_paper1_canonical_tables.py`; las tablas A7--A11, que dependen de solves exactos, se regeneran explícitamente con `scripts/analyze_paper1_p1_evidence.py --include-hardening` y quedan registradas en `models/paper1_p1_evidence_status.json`. La primera pieza convierte la historia 5K $\rightarrow$ 25K $\rightarrow$ 276K en una lectura post-selección explícita: el 5K detecta existencia, el 25K refina, y el 276K confirma el economic champion en el OOT completo.

```{python}
#| label: tbl-p1-p1-nested-confirmation
...
for candidate in [Path.cwd().resolve(), *Path.cwd().resolve().parents]:
    if (candidate / "reports" / "paper_material" / "paper1" / "tables").exists():
        ROOT = candidate
        break

tables_dir = ROOT / "reports" / "paper_material" / "paper1" / "tables"

nested = pd.read_csv(tables_dir / "paper1_tableA3_nested_holdout.csv")
nested[
    [
        "stage",
        "role",
...
```{python}
#| label: tbl-p1-p1-strict-temporal-holdout
#| tbl-cap: "Confirmación temporal estrictamente disjunta de la policy congelada"

strict_holdout = pd.read_csv(tables_dir / "paper1_tableA9_strict_temporal_holdout.csv")
strict_holdout[
    [
        "holdout_slice",
        "periods",
...
```{python}
#| label: tbl-p1-p1-decision-aware-selector
#| tbl-cap: "Selector conformal decision-aware sobre finalists existentes"

selector = pd.read_csv(tables_dir / "paper1_tableA5_decision_aware_selector.csv")
selector[
    [
        "rank",
        "partition",
...
```{python}
#| label: tbl-p1-p1-finalist-exact-eval
#| tbl-cap: "Evaluación exacta 276K de los finalists conformales"

finalist_exact = pd.read_csv(
    tables_dir / "paper1_tableA10_conformal_finalist_exact_bound_eval.csv"
)
finalist_exact[
    [
        "rank",
...
```{python}
#| label: tbl-p1-p1-segment-sensitivity
#| tbl-cap: "Segmentos periodo-grade más exigentes por cobertura 90%"

segment = pd.read_csv(tables_dir / "paper1_tableA4_segment_period_sensitivity.csv")
...
```{python}
#| label: tbl-p1-p1-funded-composition
#| tbl-cap: "Composición periodo-grade del funded set exacto"

funded_composition = pd.read_csv(tables_dir / "paper1_tableA8_funded_set_composition.csv")
...
```{python}
#| label: tbl-p1-p1-synthetic-shift
#| tbl-cap: "Stress tests sintéticos por reponderación de covariables OOT"

synthetic = pd.read_csv(tables_dir / "paper1_tableA6_synthetic_shift.csv")
synthetic[
    [
        "scenario",
        "effective_n",
...
```{python}
#| label: tbl-p1-p1-enhanced-synthetic-shift
#| tbl-cap: "Stress sintético endurecido por flips adversariales de defaults"

enhanced_shift = pd.read_csv(tables_dir / "paper1_tableA11_enhanced_synthetic_shift.csv")
enhanced_shift[
    [
        "scenario",
        "added_defaults",
