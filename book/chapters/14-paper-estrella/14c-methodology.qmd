...
El caveat editorial es importante para no sobreprometer ante IJDS: IV y WOE aquí son controles de auditabilidad y lectura de riesgo, no evidencia causal ni garantía de performance. Los umbrales IV se tratan como heurísticas de gobierno; si el manuscript menciona IV, debe hacerlo como trazabilidad de feature engineering, no como fundamento del claim CRPTO.
:::
...
Esta politica tambien evita un error metodologico frecuente en credit scoring: tratar Gini/AUC como criterio suficiente de seleccion. Wuthrich [-@wuthrich2023_gini_autocalibration] muestra que el Gini no es una regla de scoring estrictamente consistente en general porque es rank-based y no sensible a calibracion; solo recupera una lectura de seleccion defendible al restringirse a predictores auto-calibrados. Por eso el Paper Estrella reporta AUC/Gini como diagnosticos de separacion, pero selecciona y usa el score solo despues del gate de calibracion.
...
El ECE final del champion (`0.0064`) es 10× mejor que el del modelo sin calibrar (~0.07 estimado), confirmando que el resultado operacional no es accidental — la calibración sí mejora la fidelidad probabilística por bin, no sólo el rechazo formal de tests con $n$ grande (ver `tbl-p1-calibration-tests`).
:::
...
### Capa P1 de evidencia journal {#sec-estrella-p1-methodology}
...
|---|---|---|---|
| Progresión post-selección | `paper1_tableA3_nested_holdout.csv` | ¿El 276K final coincide con la cadena 5K -> 25K -> full OOT? | confirma que el economic champion no salió de una tabla aislada |
| Holdout temporal estricto | `paper1_tableA9_strict_temporal_holdout.csv` | ¿La policy congelada sobrevive a slices temporales sin solapamiento? | confirma robustez temporal sin afirmar selección prospectiva completa |
| Funded-set loan export | `paper1_tableA7_funded_set_loans.csv` | ¿Qué préstamos exactos financia el champion? | permite inspección loan-level y composición real del riesgo |
| Composición funded set | `paper1_tableA8_funded_set_composition.csv` | ¿Algún periodo/grade domina el retorno? | resume concentración, default ponderado y contribución a `V` |
| Selector decision-aware | `paper1_tableA5_decision_aware_selector.csv` | ¿El conformal winner sigue siendo razonable al mirar decisión? | rank 1 gana por gate conjunto, no solo por cobertura global |
| Exact eval finalists | `paper1_tableA10_conformal_finalist_exact_bound_eval.csv` | ¿Ranks 2/3 fueron evaluados con bound exacto? | pasan portfolio exacto, pero fallan cobertura mínima de grupo |
| Shift sintético | `paper1_tableA6_synthetic_shift.csv` | ¿La cobertura aguanta reponderaciones OOT? | primer stress de covariate shift |
| Shift endurecido | `paper1_tableA11_enhanced_synthetic_shift.csv` | ¿La cobertura aguanta flips adversariales de defaults? | stress más duro, todavía no external validation |
| Tightening condicional | `docs/research/paper_estrella_conditional_tightening_appendix_2026-05-04.md` | ¿Cuándo se pueden usar Hoeffding/Bernstein? | solo bajo independencia adicional; Markov sigue siendo principal |

: Mapa metodológico de evidencia P1 para el Paper Estrella {#tbl-p1-evidence-methodology}
...
### Estado decision-aware del diseño actual {#sec-estrella-decision-aware-status}
...
: Estado decision-aware por capa del pipeline CRPTO {#tbl-p1-decision-aware-status}
