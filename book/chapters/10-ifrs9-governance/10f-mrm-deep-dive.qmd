...
**Interpretacion**: El AUC de 0.7130 indica poder discriminante moderado-bueno, consistente con la dificultad inherente del problema (predecir default en prestamos personales no asegurados con informacion de originacion solamente). El Brier score de 0.1545 y el ECE de 0.0059 confirman buena calibracion --- el modelo no solo ordena bien los riesgos, sino que las probabilidades asignadas son confiables.
...
#| tbl-cap: "Controles operacionales del sistema conformal: 9/9 aprobados."
#| echo: false
...
if mrm:
    ...
    rows = [
        ...
        {"Control": "Ancho promedio 90%", "Valor": f"{cs['avg_width_90']:.4f}", "Umbral": "< 1.0", "Estado": "PASS"},
        {"Control": "Winkler score 90% (compensado)", "Valor": f"{cs['winkler_90']:.4f}", "Umbral": f"< {cs.get('winkler_90_compensated_threshold', 1.22)}", "Estado": "PASS"},
        ...
        {"Control": "Checks no-estadisticos", "Valor": "9/9", "Umbral": "Todos PASS", "Estado": "PASS"},
        {"Control": "overall_pass", "Valor": str(cs['overall_pass']), "Umbral": "True", "Estado": "PASS"},
...
::: {.callout-warning}
## Por que `overall_pass = True` con `strict_overall_pass = False`

Las pruebas de Kupiec y Christoffersen rechazan $H_0$ (cobertura nominal = cobertura empirica) con $p \approx 0$. Esto ocurre porque con $N = 276{,}869$ observaciones, la potencia estadistica es tan alta que detecta cualquier desviacion, por minima que sea. La desviacion observada es de +2.52% (92.52% vs 90%), que:

1. **Esta dentro del umbral de materialidad** (3%), configurado en `configs/conformal_policy.yaml`.
2. **Es conservadora**: sobre-cobertura significa que los intervalos son mas anchos de lo necesario, lo cual protege contra sub-provision.
3. **No presenta agrupamiento temporal**: la prueba de independencia de Christoffersen al 90% no rechaza ($p_{ind} = 0.187$), confirmando que las violaciones estan dispersas en el tiempo.

Esta distincion entre significancia estadistica y materialidad operativa es fundamental en MRM. Un regulador no bloquea un modelo porque un test con $N > 250{,}000$ rechaze al 5% --- lo bloquea si la desviacion tiene consecuencias economicas materiales.
:::

##### Justificacion Metodologica Formal

El sistema implementa un mecanismo de justificacion metodologica (`methodological_justification`) que permite que el veredicto global sea PASS cuando las unicas fallas son de tests estadisticos, siempre que se cumplan tres condiciones simultaneas:
...
El test Classifier Two-Sample Test (C2ST) entrena un clasificador para distinguir entre datos de entrenamiento y test. Un AUC de 0.985 indica que el clasificador puede distinguirlos casi perfectamente --- lo cual es esperado en un esquema temporal OOT donde las distribuciones cambian en el tiempo. Sin embargo, el C2ST genera solo un **warning**, no un bloqueo, porque el PSI de features (0.14 maximo) y el PSI de score (0.012) estan dentro de umbrales. El C2ST es excesivamente sensible en muestras grandes con separacion temporal; lo relevante para produccion es si el drift detectado afecta la performance, y la evidencia (delta AUC < 5%, Brier < 2%) dice que no.
:::
...
El veredicto `overall_pass = True` con 4/4 subsistemas aprobados significa que el modelo:

1. **Puede permanecer en produccion** sin necesidad de re-entrenamiento inmediato ni restricciones de uso.
2. **Los triggers de re-entrenamiento no estan activados**: PSI < 0.25, degradacion AUC < 3%, degradacion cobertura < 2%.
3. **No hay hallazgos que requieran escalamiento** al comite de riesgos como items de accion.
4. **La documentacion es trazable**: artefactos, configuracion, run tags y versiones estan registrados.

Sin embargo, 4/4 PASS **no significa ausencia de observaciones**. El reporte registra:

- 4 warnings estadisticos (Kupiec/Christoffersen) catalogados como informativos.
- C2ST warning por alta separabilidad temporal (esperada en OOT).
- Distribuciones marginales de features con drift estadisticamente significativo (pero no material por PSI).
...
La **cobertura nominal** del sistema conformal es 90%. La **cobertura empirica** observada en el test OOT es 92.52%, una desviacion de +2.52 puntos porcentuales. El umbral de materialidad configurado es 3%.

$$
\text{Desviacion} = |92.52\% - 90\%| = 2.52\% < 3\% = \text{Umbral de materialidad}
$$

Esta sobre-cobertura es **conservadora por diseno**: el sistema conformal produce intervalos ligeramente mas anchos de lo estrictamente necesario, lo cual:
...
1. **Semaforo de subsistemas**: la tabla 4/4 PASS con indicadores visuales.
...
**Nuestro caso actual**: estamos en el escenario "4/4 PASS, con observaciones informativas" (los 4 warnings estadisticos conformales y el C2ST warning). La decision correspondiente es **aprobacion con nota**, documentando las observaciones y manteniendo el monitoreo trimestral.
...
|-----------|--------|-----------|
| Pipeline predictivo | PASS (AUC 0.7130, ECE 0.006) | @sec-mrm-pipeline-subsystem |
| Sistema conformal | PASS (cobertura 92.52%, 9/9 operacionales) | @sec-mrm-conformal-subsystem |
| Gobernanza y drift | PASS (PSI < 0.25, SHAP estable) | @sec-mrm-governance-subsystem |
...
El sistema MRM implementado traduce los principios de SR 11-7 en controles concretos, automatizados y trazables. La distincion entre `overall_pass` y `strict_overall_pass` ejemplifica un tema central de la gestion de riesgo de modelo: la diferencia entre significancia estadistica y materialidad operativa. Un sistema de gobernanza maduro no solo detecta desviaciones --- las contextualiza, las documenta, y las escala solo cuando tienen consecuencias economicas.
