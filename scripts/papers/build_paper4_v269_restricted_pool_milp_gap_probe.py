#!/usr/bin/env python3
"""Build Paper 4 v269 restricted-pool MILP/gap probe artifacts."""
...
def _build_pool(
    ...
    omitted = omitted.assign(mean_return_v269=mean_returns[omitted.index.to_numpy()])
    ...
    top_ids = set(
        omitted.sort_values("mean_return_v269", ascending=False)
    ...
    selected_pool["pool_role_v269"] = "current_v266_selected"
    candidate_pool = omitted.loc[omitted["loan_id"].astype(str).isin(candidate_ids)].copy()
    candidate_pool["pool_role_v269"] = np.where(
        candidate_pool["loan_id"].astype(str).isin(frontier_add_ids),
        "omitted_candidate_from_top500_or_v268_frontier",
        "omitted_candidate_from_top500_mean_return",
    )
    keep_cols = ["loan_id", "loan_amnt", *FAMILIES, "pool_role_v269"]
...
def _update_claim_boundaries(*, improvement_found: bool, incumbent_available: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v269 restricted-pool MILP/gap probe after v268.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v269_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Restricted to current v266 selected loans plus a top-500 omitted candidate "
            ...
            {
                "claim": "v269 finds a better restricted-pool MILP incumbent than v266.",
                "allowed": bool(improvement_found and incumbent_available),
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v269_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Existence claim only inside the restricted v269 pool and solver tolerance."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v269 proves global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v269_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v269 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v269_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v269 solves a restricted-pool binary MILP over the v266 selected loans plus "
                    "top omitted candidates after v268 found no bounded two-swap improvement."
                ),
                "status": (
                ...
                "next_artifact": (
                    ...
                    if improvement_found
                    else "paper4_v270_full_universe_gap_certificate_protocol.csv"
                ),
                "success_condition": (
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V269_RESTRICTED_POOL_MILP_GAP_PROBE_START -->"
    end = "<!-- V269_RESTRICTED_POOL_MILP_GAP_PROBE_END -->"
    block = f"""
{start}
...
- Pool rows: `{status["pool_rows_v269"]}`.
- Current selected rows: `{status["current_selected_rows_v269"]}`.
- Omitted candidate rows: `{status["omitted_candidate_rows_v269"]}`.
- MILP incumbent available: `{status["milp_incumbent_available_v269"]}`.
- MILP success flag: `{status["milp_success_v269"]}`.
- MILP gap: `{status["milp_gap_v269"]}`.
- Objective delta vs v266:
  `{status["objective_delta_vs_v266_v269"]}`.
- CVaR90 delta vs v266:
  `{status["cvar90_delta_vs_v266_v269"]}`.
- Better restricted-pool incumbent found:
  `{status["restricted_pool_improvement_found_v269"]}`.
...
def main() -> None:
    ...
    if universe.empty or selected.empty or source_caps.empty:
        raise RuntimeError("Missing v55 universe, v266 selected portfolio, or source caps.")
    ...
    pool[f"milp_action_v{VERSION}"] = np.select(
        ...
        ["dropped_by_restricted_pool_milp", "added_by_restricted_pool_milp", "kept_from_v266"],
    ...
    summary = pd.DataFrame(
        [
            {
                f"probe_label_v{VERSION}": "restricted_pool_milp_after_v268_no_two_swap_improvement",
                ...
                f"objective_delta_vs_v266_v{VERSION}": objective_delta,
                f"portfolio_exposure_v{VERSION}": exposure,
                f"scenario_loss_cvar90_v{VERSION}": cvar90,
                f"cvar90_delta_vs_v266_v{VERSION}": cvar_delta,
    ...
    blockers = pd.DataFrame(
        [
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v270_full_universe_gap_certificate_protocol.csv"
                ),
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v269_restricted_pool_milp_probe_executed",
                "allowed": True,
                "artifact": "paper4_v269_restricted_pool_milp_summary.csv",
                "boundary": "restricted pool only",
            },
            {
                "claim_id": "v269_restricted_pool_improvement_found",
                "allowed": restricted_improvement,
                "artifact": "paper4_v269_restricted_pool_milp_summary.csv",
                "boundary": "inside v269 restricted pool and solver result only",
            },
            {
                "claim_id": "v269_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v269_claim_blockers.csv",
                "boundary": "full-universe gap certificate missing",
            },
            {
                "claim_id": "v269_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v269_claim_blockers.csv",
    ...
    write_csv(TABLE_DIR / "paper4_v269_restricted_pool_milp_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v269_restricted_pool_milp_source_summary.csv", source_diag)
    write_csv(TABLE_DIR / "paper4_v269_restricted_pool_milp_constraints.csv", constraints)
    write_csv(TABLE_DIR / "paper4_v269_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v269_claim_matrix_delta.csv", claim_matrix)
    ...
    row = summary.iloc[0]
    status = {
        "phase": "v269_restricted_pool_milp_gap_probe_after_v268",
        "schema_version": "2026-05-15.269",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v269": BASE_REPAIR_VERSION,
        "terminal_reprice_version_v269": ONE_SWAP_REPRICE_VERSION,
        "bounded_two_swap_version_v269": BOUNDED_TWO_SWAP_VERSION,
        "candidate_pool_limit_v269": CANDIDATE_POOL_LIMIT,
        "pool_rows_v269": int(row["pool_rows_v269"]),
        "current_selected_rows_v269": int(row["current_selected_rows_v269"]),
        "omitted_candidate_rows_v269": int(row["omitted_candidate_rows_v269"]),
        "selected_rows_v269": int(row["selected_rows_v269"]),
        "added_rows_v269": int(row["added_rows_v269"]),
        "dropped_rows_v269": int(row["dropped_rows_v269"]),
        "objective_return_v269": float(row["objective_return_v269"]),
        "objective_delta_vs_v266_v269": float(row["objective_delta_vs_v266_v269"]),
        "portfolio_exposure_v269": float(row["portfolio_exposure_v269"]),
        "scenario_loss_cvar90_v269": float(row["scenario_loss_cvar90_v269"]),
        "cvar90_delta_vs_v266_v269": float(row["cvar90_delta_vs_v266_v269"]),
        "source_cap_violations_v269": int(row["source_cap_violations_v269"]),
        "budget_feasible_v269": bool(row["budget_feasible_v269"]),
        "cvar_feasible_v269": bool(row["cvar_feasible_v269"]),
        "source_feasible_v269": bool(row["source_feasible_v269"]),
        "restricted_pool_improvement_found_v269": restricted_improvement,
        "milp_success_v269": bool(row["milp_success_v269"]),
        "milp_incumbent_available_v269": bool(row["milp_incumbent_available_v269"]),
        "milp_status_v269": int(row["milp_status_v269"]),
        "milp_gap_v269": float(row["milp_gap_v269"]),
        "milp_dual_bound_v269": float(row["milp_dual_bound_v269"]),
        "constraint_rows_v269": int(row["constraint_rows_v269"]),
        "claim_blocker_rows_v269": int(len(blockers)),
        "claim_matrix_rows_v269": int(len(claim_matrix)),
        "restricted_pool_global_optimality_claim_allowed_v269": False,
        "full_universe_integer_optimality_claim_allowed_v269": False,
        "paper1_promotion_allowed_v269": False,
        "paper4_working_champion_changed_v269": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v269 is a restricted-pool MILP/gap probe only; full-universe global "
            "and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v269_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v269": status}, indent=2, sort_keys=True))
