#!/usr/bin/env python3
"""Build Paper 4 v319 v316-specific cashflow, online, and IFRS9 gate artifacts."""
...
VERSION = 319
SOURCE_CANDIDATE_VERSION = 316
GATE_AUDIT_VERSION = 318
NEXT_VERSION = 320
...
def _template_lookup(
    *,
    ...
    raise RuntimeError("No v319 IFRS9 proxy template available.")
...
def _build_cashflow_proxy_panel(
    *,
    ...
    v316_ids = set(v316["loan_id"])
    observed = panel.loc[panel["loan_id"].isin(v316_ids)].copy()
    observed_ids = set(observed["loan_id"])
    missing = v316.loc[~v316["loan_id"].isin(observed_ids)].copy()
    ...
    templates = (
        ...
        .agg(
            **{col: (col, "mean") for col in value_cols},
            template_loan_amnt_v319=("loan_amnt", "mean"),
            donor_rows_v319=("loan_id", "nunique"),
        )
        .reset_index()
    )
        ...
        for scenario in scenarios:
            for month_index in months:
                ...
                scale = float(loan["loan_amnt"]) / max(
                    float(template["template_loan_amnt_v319"]), 1.0
                )
                row: dict[str, Any] = {
                    ...
                    f"donor_rows_v{VERSION}": int(template["donor_rows_v319"]),
                    f"imputation_scale_v{VERSION}": scale,
                    f"claim_boundary_v{VERSION}": (
                        "v316-specific IFRS9-inspired proxy imputation; not contractual IFRS9"
                    ),
                }
    ...
    observed = observed.loc[observed["loan_id"].isin(v316_ids)].copy()
    ...
    source_summary = (
        ...
        .agg(
            loan_rows_v319=("loan_id", "nunique"),
            panel_rows_v319=("loan_id", "size"),
            ecl_proxy_total_v319=("ecl_proxy_v29", "sum"),
            net_cash_proxy_total_v319=("net_cash_proxy_v47", "sum"),
        )
        .reset_index()
    )
    source_summary[f"claim_boundary_v{VERSION}"] = (
        "v319 proxy source mix only; imputed rows are not contractual IFRS9 evidence"
    )
    summary = pd.DataFrame(
        [
            {
                f"candidate_version_v{VERSION}": SOURCE_CANDIDATE_VERSION,
                f"selected_rows_v{VERSION}": int(len(v316_ids)),
                ...
                f"post_imputation_coverage_share_v{VERSION}": float(
                    full_panel["loan_id"].nunique() / max(len(v316_ids), 1)
                ),
                f"observed_coverage_share_v{VERSION}": float(
                    len(observed_ids) / max(len(v316_ids), 1)
                ),
                ...
                f"claim_boundary_v{VERSION}": (
                    "v316-specific proxy panel with explicit imputation; no contractual IFRS9 claim"
                ),
            }
        ]
    )
...
def _online_temporal_rerun(
    universe: pd.DataFrame, v316: pd.DataFrame
) -> tuple[pd.DataFrame, pd.DataFrame]:
    features = universe[FEATURE_COLS].copy()
    features["loan_id"] = features["loan_id"].astype(str)
    selected = v316[["loan_id"]].copy()
    ...
    for _, winner in winners.iterrows():
        ...
        for holdout_period, holdout in work.groupby("period", dropna=False):
            ...
            for source_id, source in holdout.groupby(source_col, dropna=False):
                rows.append(
                    {
                        f"online_method_v{VERSION}": "v65_margin_repair_replayed_on_v316",
                        ...
                        f"claim_boundary_v{VERSION}": (
                            "v316 selected-book temporal replay only; no external online claim"
                        ),
                    }
                )
    ...
    summary[f"claim_boundary_v{VERSION}"] = (
        "v316 temporal selected-book replay; external holdout required for live claim"
    )
...
def _update_claim_boundaries() -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v319 v316-specific cashflow proxy panel.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v319_v316_cashflow_proxy_summary.csv"
                ),
            ...
            {
                "claim": "Paper 4 has a v319 v316 temporal online rerun.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v319_v316_online_temporal_summary.csv"
                ),
            ...
            {
                "claim": "v319 implements contractual IFRS9 for v316.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v319_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v319 authorizes live online deployability for v316.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v319_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v319 resolves v316 global or matched-period dynamic blockers.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v319_claim_blockers.csv"
                ),
                "boundary": "v319 is an online/cashflow/proxy gate, not a global certificate.",
                "prohibited_claim_flag": True,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v319 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v319_claim_blockers.csv"
                ),
...
def _update_backlog() -> None:
        ...
        [
            {
                ...
                "executable_item": (
                    "v319 builds a v316-specific cashflow proxy panel and temporal online rerun."
                ),
                "status": "v316_cashflow_proxy_and_temporal_online_rerun_executed",
                "next_artifact": (
                    f"paper4_v{NEXT_VERSION}_v316_matched_period_or_branch_price_protocol.csv"
                ),
                "success_condition": (
                    "reduce imputation, external holdout, matched-period or global blockers "
                    "without promoting"
                ),
                "last_wave": "v319",
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v319")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V319_V316_CASHFLOW_ONLINE_IFRS9_GATE_START -->"
    end = "<!-- V319_V316_CASHFLOW_ONLINE_IFRS9_GATE_END -->"
    block = f"""
{start}
...
- Cashflow proxy panel rows: `{status["cashflow_proxy_panel_rows_v319"]}`.
- Observed proxy loan rows: `{status["observed_proxy_loan_rows_v319"]}`.
- Imputed proxy loan rows: `{status["imputed_proxy_loan_rows_v319"]}`.
- Post-imputation coverage share:
  `{status["post_imputation_coverage_share_v319"]}`.
- Online temporal cell rows: `{status["online_temporal_cell_rows_v319"]}`.
- Online internal all-gate family rows:
  `{status["online_internal_all_gate_family_rows_v319"]}`.
- Strict live deployability claim allowed:
  `{status["strict_live_deployability_claim_allowed_v319"]}`.
- Contractual IFRS9 claim allowed:
  `{status["contractual_ifrs9_claim_allowed_v319"]}`.
- Working champion claim allowed:
  `{status["working_champion_claim_allowed_v319"]}`.
...
def main() -> None:
    ...
    v318_status = json.loads((STATUS_DIR / "paper4_v318_status.json").read_text(encoding="utf-8"))
    if universe.empty or v316.empty:
        raise RuntimeError("Missing v55 or v316 inputs for v319.")
    if int(v318_status["v316_missing_v47_proxy_rows_v318"]) <= 0:
        raise RuntimeError("v319 expects v318 to identify v316 IFRS9 proxy coverage gaps.")

    cashflow_panel, cashflow_summary, proxy_source_summary = _build_cashflow_proxy_panel(
        universe=universe,
        v316=v316,
    )
    online_cells, online_summary = _online_temporal_rerun(universe=universe, v316=v316)
    ...
    gate_summary = pd.DataFrame(
        [
            {
                f"gate_id_v{VERSION}": "v319_v316_cashflow_online_ifrs9_gate",
                f"candidate_version_v{VERSION}": SOURCE_CANDIDATE_VERSION,
                ...
                f"claim_boundary_v{VERSION}": (
                    "v316 candidate-specific cashflow/online/IFRS9 proxy gate; not live or contractual"
                ),
            }
        ]
    )
        ...
        [
            {
                ...
                f"required_next_artifact_v{VERSION}": "future_observed_v316_cashflow_panel",
            ...
            },
            {
                ...
                f"claim_boundary_v{VERSION}": "v319 online replay is internal selected-book only",
            },
            {
                f"blocker_id_v{VERSION}": "matched_period_dynamic_gap_from_v318",
                f"blocking_v{VERSION}": True,
                f"evidence_count_v{VERSION}": int(
                    not bool(v318_status["period_distribution_match_v318"])
                ),
                f"required_next_artifact_v{VERSION}": "future_matched_period_dynamic_replay",
                f"claim_boundary_v{VERSION}": "v319 does not repair the v318 period mismatch",
            },
            {
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v319_v316_cashflow_proxy_panel_executed",
                "allowed": True,
                "artifact": "paper4_v319_v316_cashflow_proxy_summary.csv",
                "boundary": "proxy panel with imputation",
            },
            {
                "claim_id": "v319_v316_online_temporal_rerun_executed",
                "allowed": True,
                "artifact": "paper4_v319_v316_online_temporal_summary.csv",
                "boundary": "internal selected-book replay",
            },
            {
                "claim_id": "v319_v316_internal_online_all_gates",
                "allowed": online_all_gate_rows == len(online_summary),
                "artifact": "paper4_v319_v316_online_temporal_summary.csv",
                "boundary": "internal historical replay only, not live deployability",
            },
            {
                "claim_id": "v319_contractual_ifrs9",
                "allowed": False,
                "artifact": "paper4_v319_claim_blockers.csv",
                "boundary": "imputed proxy rows and missing servicing panel",
            },
            {
                "claim_id": "v319_strict_live_online_deployability",
                "allowed": False,
                "artifact": "paper4_v319_claim_blockers.csv",
                "boundary": "external holdout missing",
            },
            {
                "claim_id": "v319_working_champion_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v319_claim_blockers.csv",
    ...
    write_csv(TABLE_DIR / "paper4_v319_v316_cashflow_online_ifrs9_gate.csv", gate_summary)
    cashflow_panel.to_parquet(
        TABLE_DIR / "paper4_v319_v316_cashflow_proxy_panel.parquet", index=False
    )
    write_csv(TABLE_DIR / "paper4_v319_v316_cashflow_proxy_summary.csv", cashflow_summary)
    write_csv(
        TABLE_DIR / "paper4_v319_v316_cashflow_proxy_source_summary.csv", proxy_source_summary
    )
    write_csv(TABLE_DIR / "paper4_v319_v316_online_temporal_cells.csv", online_cells)
    write_csv(TABLE_DIR / "paper4_v319_v316_online_temporal_summary.csv", online_summary)
    write_csv(TABLE_DIR / "paper4_v319_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v319_claim_matrix_delta.csv", claim_matrix)
    _update_claim_boundaries()
    _update_backlog()

    gate_row = gate_summary.iloc[0]
    status = {
        "phase": "v319_v316_cashflow_online_ifrs9_gate",
        "schema_version": "2026-05-16.319",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "source_candidate_version_v319": SOURCE_CANDIDATE_VERSION,
        "gate_audit_version_v319": GATE_AUDIT_VERSION,
        "selected_rows_v319": int(gate_row[f"selected_rows_v{VERSION}"]),
        "cashflow_proxy_panel_rows_v319": int(gate_row[f"cashflow_proxy_panel_rows_v{VERSION}"]),
        "observed_proxy_loan_rows_v319": int(gate_row[f"observed_proxy_loan_rows_v{VERSION}"]),
        "imputed_proxy_loan_rows_v319": int(gate_row[f"imputed_proxy_loan_rows_v{VERSION}"]),
        "post_imputation_coverage_share_v319": float(
            gate_row[f"post_imputation_coverage_share_v{VERSION}"]
        ),
        "observed_coverage_share_v319": float(gate_row[f"observed_coverage_share_v{VERSION}"]),
        "cashflow_proxy_months_v319": int(gate_row[f"cashflow_proxy_months_v{VERSION}"]),
        "cashflow_proxy_scenarios_v319": int(gate_row[f"cashflow_proxy_scenarios_v{VERSION}"]),
        "cashflow_proxy_source_rows_v319": int(gate_row[f"cashflow_proxy_source_rows_v{VERSION}"]),
        "online_temporal_cell_rows_v319": int(gate_row[f"online_temporal_cell_rows_v{VERSION}"]),
        "online_temporal_summary_rows_v319": int(
            gate_row[f"online_temporal_summary_rows_v{VERSION}"]
        ),
        "online_internal_all_gate_family_rows_v319": online_all_gate_rows,
        "strict_live_deployability_claim_allowed_v319": strict_live_allowed,
        "contractual_ifrs9_claim_allowed_v319": contractual_ifrs9_allowed,
        "working_champion_claim_allowed_v319": working_champion_allowed,
        "full_universe_integer_optimality_claim_allowed_v319": False,
        "paper1_promotion_allowed_v319": False,
        "paper4_working_champion_changed_v319": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_blocker_rows_v319": int(len(blockers)),
        "claim_matrix_rows_v319": int(len(claim_matrix)),
        "next_artifact_v319": gate_row[f"next_artifact_v{VERSION}"],
        "claim_boundary": (
            "v319 builds v316-specific proxy cashflow and online temporal rerun artifacts; "
            "contractual IFRS9, strict live deployability, working champion, and promotion remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v319_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v319": status}, indent=2, sort_keys=True))
