#!/usr/bin/env python3
"""Build Paper 4 v359 expanded v353 branch-price / dual-bound artifacts."""
...
NEXT_ARTIFACT = (
    f"paper4_v{NEXT_VERSION}_v353_apply_expanded_branch_price_candidate_or_bound_memo.csv"
)
...
def _unique_two_swap_seeds(candidate_screen: pd.DataFrame) -> pd.DataFrame:
    ...
    ordered = ordered.sort_values(
        [
            f"cvar90_after_two_swap_v{PRIOR_BRANCH_VERSION}",
    ...
    *,
    ...
    for add_pos, drop_pos in source_exact_positions:
        ...
        candidate_rows.append(
            {
                "policy_id": "v359_v353_expanded_branch_price_loop",
                f"regime_v{VERSION}": "post_v353_seeded_three_add_three_drop",
                ...
                f"action_signature_v{VERSION}": v352._action_signature(all_add_ids, all_drop_ids),
                ...
                f"claim_boundary_v{VERSION}": (
                    "expanded bounded post-v353 third-order branch-price row; "
    ...
    stage_row = {
        ...
        f"claim_boundary_v{VERSION}": (
            "one unique v357 source-exact two-swap row used as a third-order seed"
        ),
    }
    for family in FAMILIES:
...
def _stage_summary(
    *,
    ...
    rows: list[dict[str, Any]] = [
        {
            f"stage_v{VERSION}": "v357_unique_source_exact_two_swap_seeds",
            f"row_count_v{VERSION}": int(len(seed_stage)),
            f"claim_boundary_v{VERSION}": "unique v357 source-exact two-swap actions",
        },
        {
...
def _update_claim_boundaries(*, entering_rows: int) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "v359 executes an expanded post-v353 third-order branch-price loop.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v359_v353_expand_branch_price_or_full_dual_bound.csv"
                ),
                "boundary": (
                    "Unique v357 two-swap seeds plus one extra source-tight add/drop; "
                    "no dual-bound certificate."
                ),
            ...
            {
                "claim": "v359 finds expanded post-v353 CVaR-feasible entering candidates.",
                "allowed": entering_rows > 0,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v359_entering_candidate_summary.csv"
                ),
                "boundary": (
                    "Bounded v357-seeded third-order scope only; apply/reprice required "
            ...
            {
                "claim": "v359 finds no expanded post-v353 CVaR-feasible entering column.",
            ...
            {
                "claim": "v359 proves a valid full-universe branch-price bound.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v359_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v359 authorizes a Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v359_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v359 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v359_claim_blockers.csv"
                ),
...
def _update_backlog(*, entering_rows: int) -> None:
    ...
    status = (
        "expanded_post_v353_branch_price_entering_candidate_found_requires_apply_reprice"
        if entering_rows
        else "expanded_post_v353_branch_price_loop_no_cvar_entering_column"
    )
    result = (
        "bounded_post_v353_third_order_cvar_feasible_entering_column_found"
        if entering_rows
        else "no_bounded_post_v353_third_order_cvar_feasible_entering_column"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "executable_item": (
                    "v359 expands the v357 bounded branch-price loop from unique "
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v359")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V359_V353_EXPAND_BRANCH_PRICE_OR_FULL_DUAL_BOUND_START -->"
    end = "<!-- V359_V353_EXPAND_BRANCH_PRICE_OR_FULL_DUAL_BOUND_END -->"
    best_entering = status["best_entering_return_delta_v359"]
    best_entering_text = (
...
- Unique v357 two-swap seed rows:
  `{status["two_swap_seed_rows_v359"]}`.
- Positive source-tight third-add candidates:
  `{status["positive_source_tight_candidate_rows_v359"]}`.
- Ordered third-order rows screened:
  `{status["ordered_third_order_rows_screened_v359"]}`.
- Budget+return feasible rows:
  `{status["budget_return_feasible_rows_v359"]}`.
- Source-exact third-order rows:
  `{status["source_exact_third_order_rows_v359"]}`.
- Unique source-exact action signatures:
  `{status["unique_source_exact_action_signatures_v359"]}`.
- CVaR-feasible entering rows:
  `{status["cvar_feasible_entering_rows_v359"]}`.
- Best entering return delta:
  `{best_entering_text}`.
- Best entering CVaR90:
  `{status["best_entering_cvar90_v359"]}`.
- Valid branch-price bound:
  `{status["valid_branch_price_bound_v359"]}`.
...
def main() -> None:
    ...
    v358_memo = read_csv("paper4_v358_v353_apply_branch_price_candidate_or_bound_memo.csv")
    v358_status = json.loads((STATUS_DIR / "paper4_v358_status.json").read_text(encoding="utf-8"))
    if any(
        df.empty
        for df in [
            universe,
        ...
        raise RuntimeError("Missing v359 expanded branch-price inputs.")
    if not bool(v358_status["no_apply_disposition_allowed_v358"]):
        raise RuntimeError("v359 expects v358 no-apply disposition after v357 no-entry evidence.")
    ...
    two_swap_seeds = _unique_two_swap_seeds(v357_candidates)
    ...
    protocol = pd.DataFrame(
        [
            {
                f"protocol_id_v{VERSION}": "v353_expanded_third_order_branch_price_loop",
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v359_expanded_branch_price_loop_executed",
                "allowed": True,
                "artifact": "paper4_v359_v353_expand_branch_price_or_full_dual_bound.csv",
                "boundary": "expanded bounded third-order scope",
            },
            {
                "claim_id": "v359_bounded_entering_candidate_found",
                "allowed": entering_rows > 0,
                "artifact": "paper4_v359_entering_candidate_summary.csv",
                "boundary": "bounded candidate only; apply/reprice required",
            },
            {
                "claim_id": "v359_no_expanded_third_order_entering_column",
                "allowed": entering_rows == 0,
                "artifact": "paper4_v359_third_order_branch_price_stage_summary.csv",
                "boundary": "not a global branch-price termination",
            },
            {
                "claim_id": "v359_valid_full_universe_branch_price_bound",
                "allowed": False,
                "artifact": "paper4_v359_claim_blockers.csv",
                "boundary": "dual-bound loop missing",
            },
            {
                "claim_id": "v359_working_champion_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v359_claim_blockers.csv",
    ...
    write_csv(TABLE_DIR / "paper4_v359_v353_expand_branch_price_or_full_dual_bound.csv", protocol)
    write_csv(TABLE_DIR / "paper4_v359_third_order_seed_summary.csv", seed_stage)
    write_csv(TABLE_DIR / "paper4_v359_third_order_branch_price_stage_summary.csv", stage_summary)
    write_csv(TABLE_DIR / "paper4_v359_branch_price_candidate_screen.csv", candidate_screen)
    write_csv(TABLE_DIR / "paper4_v359_entering_candidate_summary.csv", entering_summary)
    write_csv(TABLE_DIR / "paper4_v359_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v359_claim_matrix_delta.csv", claim_matrix)
    _update_claim_boundaries(entering_rows=entering_rows)
    _update_backlog(entering_rows=entering_rows)

    row = protocol.iloc[0]
    status = {
        "phase": "v359_v353_expand_branch_price_or_full_dual_bound",
        "schema_version": "2026-05-17.359",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_version_v359": BASE_VERSION,
        "prior_branch_version_v359": PRIOR_BRANCH_VERSION,
        "readiness_version_v359": READINESS_VERSION,
        "disposition_version_v359": DISPOSITION_VERSION,
        "selected_rows_v359": int(row[f"selected_rows_v{VERSION}"]),
        "two_swap_seed_rows_v359": int(row[f"two_swap_seed_rows_v{VERSION}"]),
        "positive_source_tight_candidate_rows_v359": int(
            row[f"positive_source_tight_candidate_rows_v{VERSION}"]
        ),
        "ordered_third_order_rows_screened_v359": int(
            row[f"ordered_third_order_rows_screened_v{VERSION}"]
        ),
        "return_improving_rows_v359": int(row[f"return_improving_rows_v{VERSION}"]),
        "budget_return_feasible_rows_v359": int(row[f"budget_return_feasible_rows_v{VERSION}"]),
        "grade_source_feasible_rows_v359": int(row[f"grade_source_feasible_rows_v{VERSION}"]),
        "score_decile_source_feasible_rows_v359": int(
            row[f"score_decile_source_feasible_rows_v{VERSION}"]
        ),
        "source_exact_third_order_rows_v359": int(row[f"source_exact_third_order_rows_v{VERSION}"]),
        "unique_source_exact_action_signatures_v359": unique_source_exact,
        "cvar_feasible_entering_rows_v359": entering_rows,
        "entering_candidate_summary_rows_v359": int(
            row[f"entering_candidate_summary_rows_v{VERSION}"]
        ),
        "best_source_exact_return_delta_v359": _maybe_float(
            row[f"best_source_exact_return_delta_v{VERSION}"]
        ),
        "best_source_exact_cvar90_v359": _maybe_float(row[f"best_source_exact_cvar90_v{VERSION}"]),
        "best_entering_return_delta_v359": _maybe_float(
            row[f"best_entering_return_delta_v{VERSION}"]
        ),
        "best_entering_cvar90_v359": _maybe_float(row[f"best_entering_cvar90_v{VERSION}"]),
        "branch_price_loop_executed_v359": True,
        "valid_branch_price_bound_v359": False,
        "full_universe_integer_optimality_claim_allowed_v359": False,
        "working_champion_claim_allowed_v359": False,
        "paper1_promotion_allowed_v359": False,
        "paper4_working_champion_changed_v359": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_blocker_rows_v359": int(len(blockers)),
        "claim_matrix_rows_v359": int(len(claim_matrix)),
        "next_artifact_v359": NEXT_ARTIFACT,
        "claim_boundary": (
            "v359 expands the bounded branch-price loop; full dual-bound, proxy, live, "
            "champion and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v359_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v359": status}, indent=2, sort_keys=True))
