#!/usr/bin/env python3
"""Build Paper 4 v259 post-v258 one-swap repricing artifacts."""
...
NEXT_VERSION = 260
REPRICE_SCOPE = "post_v258_bounded_two_swap_one_drop_one_add_whole_loan_swap"
...
def _candidate_pairs_for_reprice() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    selected = read_parquet("paper4_v258_bounded_two_swap_repair_allocations.parquet")
    ...
    repair_summary = read_csv("paper4_v258_bounded_two_swap_repair_summary.csv")
    repair_row = repair_summary.loc[
        repair_summary["portfolio_label_v258"].eq("bounded_two_swap_repair_candidate")
    ].iloc[0]
    action = read_csv("paper4_v258_bounded_two_swap_repair_action.csv")
    ...
    policy_id = (
        str(action["policy_id"].iloc[0]) if not action.empty else "v258_bounded_two_swap_candidate"
    )
    regime = "post_v258_bounded_two_swap_repair"
    current_exposure = float(repair_row["portfolio_exposure_v258"])
    exposure_min = float(repair_row["exposure_min_v258"])
    exposure_max = float(repair_row["exposure_max_v258"])
    cvar_cap = float(repair_row["cvar_cap_v258"])
    current_objective_return = float(repair_row["objective_return_v258"])
    ...
    for candidate_pos, selected_pos in np.argwhere(source_prefilter_mask):
        ...
        row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "post-v258 bounded two-swap one-swap pricing only; not multi-swap or global proof"
            ),
        }
    ...
    local_claim_boundary = (
        "post-v258 one-swap screen cleared; broader multi-swap/global proof still missing"
        if improving_pairs == 0
        else ("post-v258 one-swap screen only; repeat repair/repricing because improvements remain")
    )
    ...
    stage_summary[f"claim_boundary_v{VERSION}"] = (
        "post-v258 bounded two-swap one-swap screen stage count only"
    )
...
def _claim_blockers(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    improvement_next_artifact = (
        "paper4_v260_apply_next_one_swap_after_bounded_two_swap_repair.csv"
        if improving > 0
        else "paper4_v260_broader_multi_swap_or_global_gap_probe.csv"
    )
    improvement_boundary = (
        "feasible improving post-v258 one-swaps block local optimality"
        if improving > 0
        else "no feasible improving post-v258 one-swaps remain"
    )
    return pd.DataFrame(
        [
            {
                "blocker_id_v259": "post_v258_one_swap_improvement_found",
                "blocking_v259": improving > 0,
                "evidence_count_v259": improving,
                "required_next_artifact_v259": improvement_next_artifact,
                "claim_boundary_v259": improvement_boundary,
            },
            {
                "blocker_id_v259": "broader_multi_swap_search_not_exhaustive",
                "blocking_v259": True,
                "evidence_count_v259": 1,
                "required_next_artifact_v259": (
                    "paper4_v260_broader_multi_swap_or_global_gap_probe.csv"
                ),
                "claim_boundary_v259": (
                    "post-v258 one-swap screen is not an exhaustive multi-swap proof"
                ),
            },
            {
                "blocker_id_v259": "global_integer_gap_certificate_missing",
                "blocking_v259": True,
                "evidence_count_v259": 1,
                "required_next_artifact_v259": "paper4_v260_global_integer_gap_protocol.csv",
                "claim_boundary_v259": "no branch-and-price/global full-universe certificate",
            },
            {
                "blocker_id_v259": "paper4_final_promotion_forbidden",
                "blocking_v259": True,
                "evidence_count_v259": 1,
                "required_next_artifact_v259": "paper4_final_promotion_gate_not_created",
                "claim_boundary_v259": (
...
def _claim_matrix(*, local_optimality_cleared: bool) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v259_post_v258_bounded_two_swap_one_swap_reprice_executed",
                "allowed": True,
                "artifact": "paper4_v259_post_bounded_two_swap_one_swap_summary.csv",
                "boundary": "post-v258 one-drop/one-add screen completed",
            },
            {
                "claim_id": "v259_post_v258_one_swap_local_optimality",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v259_claim_blockers.csv",
                "boundary": "allowed only when no feasible improving post-v258 one-swaps remain",
            },
            {
                "claim_id": "v259_multi_swap_or_global_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v259_claim_blockers.csv",
                "boundary": "broader multi-swap/global gap evidence missing",
            },
            {
                "claim_id": "v259_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v259_claim_blockers.csv",
...
def _update_claim_boundaries(*, local_optimality_cleared: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": ("Paper 4 has a v259 post-v258 bounded two-swap one-swap pricing screen."),
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v259_post_bounded_two_swap_one_swap_summary.csv"
                ),
                "boundary": (
                    "One-drop/one-add screen after v258 repair; not global integer proof."
                ),
            ...
            {
                "claim": (
                    "v259 proves the v258 bounded two-swap repaired portfolio is "
                    "one-swap locally optimal."
                ),
                "allowed": local_optimality_cleared,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v259_claim_blockers.csv"
                ),
                "boundary": ("Allowed only if no feasible improving post-v258 one-swaps remain."),
                "prohibited_claim_flag": not local_optimality_cleared,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v259 proves multi-swap or global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v259_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v259 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v259_claim_blockers.csv"
                ),
...
def _update_backlog(*, local_optimality_cleared: bool) -> None:
    ...
    next_artifact = (
        ...
        else "paper4_v260_apply_next_one_swap_after_bounded_two_swap_repair.csv"
    )
    status = (
    ...
    execution_result = (
        "post_v258_one_swap_reprice_cleared"
        if local_optimality_cleared
        else "post_v258_one_swap_reprice_found_improvements"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v259 reruns one-swap pricing after the v258 bounded two-swap "
                    "repair over all non-selected comparable loans."
                ),
                "status": status,
                "next_artifact": next_artifact,
                "success_condition": (
                    "no feasible improving post-v258 one-swaps remain before moving to "
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V259_POST_V258_BOUNDED_TWO_SWAP_ONE_SWAP_REPRICE_START -->"
    end = "<!-- V259_POST_V258_BOUNDED_TWO_SWAP_ONE_SWAP_REPRICE_END -->"
    best_delta = status["best_one_swap_return_delta_v259"]
    best_delta_text = (
...
- Pair rows screened: `{status["total_pair_rows_screened_v259"]}`.
- Candidate add rows: `{status["candidate_add_rows_v259"]}`.
- Return-improving pairs: `{status["return_improving_pair_rows_v259"]}`.
- Exact source-feasible pairs: `{status["source_exact_pair_rows_v259"]}`.
- CVaR-feasible improving one-swaps: `{status["one_swap_improving_rows_v259"]}`.
- Best post-v258 one-swap return delta:
  `{best_delta_text}`.
- Post-v258 one-swap local optimality cleared:
  `{status["post_bounded_two_swap_one_swap_local_optimality_cleared_v259"]}`.
...
def main() -> None:
    ...
    write_csv(
        TABLE_DIR / "paper4_v259_post_bounded_two_swap_one_swap_reprice.csv",
        pairs,
    )
    write_csv(
        TABLE_DIR / "paper4_v259_post_bounded_two_swap_one_swap_top_candidates.csv",
        top_candidates,
    )
    write_csv(
        TABLE_DIR / "paper4_v259_post_bounded_two_swap_one_swap_summary.csv",
        summary,
    )
    write_csv(
        TABLE_DIR / "paper4_v259_post_bounded_two_swap_one_swap_stage_summary.csv",
        stage_summary,
    )
    write_csv(TABLE_DIR / "paper4_v259_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v259_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v259_post_v258_bounded_two_swap_one_swap_reprice",
        "schema_version": "2026-05-15.259",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "summary_rows_v259": int(len(summary)),
        "stage_summary_rows_v259": int(len(stage_summary)),
        "candidate_pair_rows_v259": int(len(pairs)),
        "top_candidate_rows_v259": int(len(top_candidates)),
        "claim_blocker_rows_v259": int(len(blockers)),
        "claim_matrix_rows_v259": int(len(claim_matrix)),
        "selected_rows_v259": int(row["selected_rows_v259"]),
        "candidate_add_rows_v259": int(row["candidate_add_rows_v259"]),
        "total_pair_rows_screened_v259": int(row["total_pair_rows_screened_v259"]),
        "return_improving_pair_rows_v259": int(row["return_improving_pair_rows_v259"]),
        "budget_return_feasible_pair_rows_v259": int(row["budget_return_feasible_pair_rows_v259"]),
        "source_prefilter_pair_rows_v259": int(row["source_prefilter_pair_rows_v259"]),
        "source_exact_pair_rows_v259": int(row["source_exact_pair_rows_v259"]),
        "cvar_feasible_pair_rows_v259": int(row["cvar_feasible_pair_rows_v259"]),
        "one_swap_improving_rows_v259": int(row["one_swap_improving_rows_v259"]),
        "best_one_swap_return_delta_v259": (
            None if pd.isna(best_return_delta) else float(best_return_delta)
        ),
        "best_one_swap_cvar90_after_v259": (
            None if pd.isna(best_cvar90_after) else float(best_cvar90_after)
        ),
        "current_exposure_v259": float(row["current_exposure_v259"]),
        "current_objective_return_v259": float(row["current_objective_return_v259"]),
        "post_bounded_two_swap_one_swap_local_optimality_cleared_v259": (local_optimality_cleared),
        "full_universe_integer_optimality_claim_allowed_v259": False,
        "paper1_promotion_allowed_v259": False,
        "paper4_working_champion_changed_v259": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v259 is post-v258 bounded two-swap one-swap pricing only; "
            "multi-swap/global and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v259_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v259": status}, indent=2, sort_keys=True))
