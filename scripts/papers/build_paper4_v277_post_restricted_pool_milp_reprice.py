#!/usr/bin/env python3
"""Build Paper 4 v277 post-v276 one-swap repricing artifacts."""
...
VERSION = 277
PREVIOUS_REPAIR_VERSION = 276
NEXT_VERSION = 278
REPRICE_SCOPE = "post_v276_restricted_pool_milp_one_drop_one_add_whole_loan_swap"
...
def _candidate_pairs_for_reprice() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    repair_row = repair_summary.loc[
        repair_summary["portfolio_label_v276"].eq("restricted_pool_milp_repair_candidate")
    ].iloc[0]
    ...
    policy_id = "v276_restricted_pool_milp_repair_candidate"
    regime = "post_v276_restricted_pool_milp_repair"
    current_exposure = float(repair_row["portfolio_exposure_v276"])
    exposure_min = float(repair_row["exposure_min_v276"])
    exposure_max = float(repair_row["exposure_max_v276"])
    cvar_cap = float(repair_row["cvar_cap_v276"])
    current_objective_return = float(repair_row["objective_return_v276"])
    ...
    for candidate_pos, selected_pos in np.argwhere(source_prefilter_mask):
        ...
        row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "post-v276 restricted-pool MILP one-swap pricing only; not multi-swap or global proof"
            ),
        }
    ...
    local_claim_boundary = (
        "post-v276 one-swap screen cleared; broader multi-swap/global proof still missing"
        if improving_pairs == 0
        else ("post-v276 one-swap screen only; repeat repair/repricing because improvements remain")
    )
    ...
    stage_summary[f"claim_boundary_v{VERSION}"] = (
        "post-v276 restricted-pool MILP one-swap screen stage count only"
    )
...
def _claim_blockers(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    improvement_next_artifact = (
        "paper4_v278_apply_next_one_swap_after_restricted_pool_milp_repair.csv"
        if improving > 0
        else "paper4_v278_broader_multi_swap_or_global_gap_probe.csv"
    )
    improvement_boundary = (
        "feasible improving post-v276 one-swaps block local optimality"
        if improving > 0
        else "no feasible improving post-v276 one-swaps remain"
    )
    return pd.DataFrame(
        [
            {
                "blocker_id_v277": "post_v276_one_swap_improvement_found",
                "blocking_v277": improving > 0,
                "evidence_count_v277": improving,
                "required_next_artifact_v277": improvement_next_artifact,
                "claim_boundary_v277": improvement_boundary,
            },
            {
                "blocker_id_v277": "broader_multi_swap_search_not_exhaustive",
                "blocking_v277": True,
                "evidence_count_v277": 1,
                "required_next_artifact_v277": (
                    "paper4_v278_broader_multi_swap_or_global_gap_probe.csv"
                ),
                "claim_boundary_v277": (
                    "post-v276 one-swap screen is not an exhaustive multi-swap proof"
                ),
            },
            {
                "blocker_id_v277": "global_integer_gap_certificate_missing",
                "blocking_v277": True,
                "evidence_count_v277": 1,
                "required_next_artifact_v277": "paper4_v278_global_integer_gap_protocol.csv",
                "claim_boundary_v277": "no branch-and-price/global full-universe certificate",
            },
            {
                "blocker_id_v277": "paper4_final_promotion_forbidden",
                "blocking_v277": True,
                "evidence_count_v277": 1,
                "required_next_artifact_v277": "paper4_final_promotion_gate_not_created",
                "claim_boundary_v277": (
...
def _claim_matrix(*, local_optimality_cleared: bool) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v277_post_v276_restricted_pool_milp_one_swap_reprice_executed",
                "allowed": True,
                "artifact": "paper4_v277_post_restricted_pool_milp_one_swap_summary.csv",
                "boundary": "post-v276 one-drop/one-add screen completed",
            },
            {
                "claim_id": "v277_post_v276_one_swap_local_optimality",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v277_claim_blockers.csv",
                "boundary": "allowed only when no feasible improving post-v276 one-swaps remain",
            },
            {
                "claim_id": "v277_multi_swap_or_global_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v277_claim_blockers.csv",
                "boundary": "broader multi-swap/global gap evidence missing",
            },
            {
                "claim_id": "v277_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v277_claim_blockers.csv",
...
def _update_claim_boundaries(*, local_optimality_cleared: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": (
                    "Paper 4 has a v277 post-v276 restricted-pool MILP one-swap pricing screen."
                ),
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v277_post_restricted_pool_milp_one_swap_summary.csv"
                ),
                "boundary": (
                    "One-drop/one-add screen after v276 repair; not global integer proof."
                ),
            ...
            {
                "claim": (
                    "v277 proves the v276 restricted-pool MILP repaired portfolio is "
                    "one-swap locally optimal."
                ),
                "allowed": local_optimality_cleared,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v277_claim_blockers.csv"
                ),
                "boundary": ("Allowed only if no feasible improving post-v276 one-swaps remain."),
                "prohibited_claim_flag": not local_optimality_cleared,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v277 proves multi-swap or global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v277_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v277 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v277_claim_blockers.csv"
                ),
...
def _update_backlog(*, local_optimality_cleared: bool) -> None:
    ...
    next_artifact = (
        ...
        else "paper4_v278_apply_next_one_swap_after_restricted_pool_milp_repair.csv"
    )
    status = (
    ...
    execution_result = (
        "post_v276_one_swap_reprice_cleared"
        if local_optimality_cleared
        else "post_v276_one_swap_reprice_found_improvements"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v277 reruns one-swap pricing after the v276 restricted-pool MILP "
                    "repair over all non-selected comparable loans."
                ),
                "status": status,
                "next_artifact": next_artifact,
                "success_condition": (
                    "no feasible improving post-v276 one-swaps remain before moving to "
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V277_POST_V276_RESTRICTED_POOL_MILP_ONE_SWAP_REPRICE_START -->"
    end = "<!-- V277_POST_V276_RESTRICTED_POOL_MILP_ONE_SWAP_REPRICE_END -->"
    best_delta = status["best_one_swap_return_delta_v277"]
    best_delta_text = (
...
- Pair rows screened: `{status["total_pair_rows_screened_v277"]}`.
- Candidate add rows: `{status["candidate_add_rows_v277"]}`.
- Return-improving pairs: `{status["return_improving_pair_rows_v277"]}`.
- Exact source-feasible pairs: `{status["source_exact_pair_rows_v277"]}`.
- CVaR-feasible improving one-swaps: `{status["one_swap_improving_rows_v277"]}`.
- Best post-v276 one-swap return delta:
  `{best_delta_text}`.
- Post-v276 one-swap local optimality cleared:
  `{status["post_restricted_pool_milp_one_swap_local_optimality_cleared_v277"]}`.
...
def main() -> None:
    ...
    write_csv(
        TABLE_DIR / "paper4_v277_post_restricted_pool_milp_one_swap_reprice.csv",
        pairs,
    )
    write_csv(
        TABLE_DIR / "paper4_v277_post_restricted_pool_milp_one_swap_top_candidates.csv",
        top_candidates,
    )
    write_csv(
        TABLE_DIR / "paper4_v277_post_restricted_pool_milp_one_swap_summary.csv",
        summary,
    )
    write_csv(
        TABLE_DIR / "paper4_v277_post_restricted_pool_milp_one_swap_stage_summary.csv",
        stage_summary,
    )
    write_csv(TABLE_DIR / "paper4_v277_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v277_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v277_post_v276_restricted_pool_milp_one_swap_reprice",
        "schema_version": "2026-05-15.277",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "summary_rows_v277": int(len(summary)),
        "stage_summary_rows_v277": int(len(stage_summary)),
        "candidate_pair_rows_v277": int(len(pairs)),
        "top_candidate_rows_v277": int(len(top_candidates)),
        "claim_blocker_rows_v277": int(len(blockers)),
        "claim_matrix_rows_v277": int(len(claim_matrix)),
        "selected_rows_v277": int(row["selected_rows_v277"]),
        "candidate_add_rows_v277": int(row["candidate_add_rows_v277"]),
        "total_pair_rows_screened_v277": int(row["total_pair_rows_screened_v277"]),
        "return_improving_pair_rows_v277": int(row["return_improving_pair_rows_v277"]),
        "budget_return_feasible_pair_rows_v277": int(row["budget_return_feasible_pair_rows_v277"]),
        "source_prefilter_pair_rows_v277": int(row["source_prefilter_pair_rows_v277"]),
        "source_exact_pair_rows_v277": int(row["source_exact_pair_rows_v277"]),
        "cvar_feasible_pair_rows_v277": int(row["cvar_feasible_pair_rows_v277"]),
        "one_swap_improving_rows_v277": int(row["one_swap_improving_rows_v277"]),
        "best_one_swap_return_delta_v277": (
            None if pd.isna(best_return_delta) else float(best_return_delta)
        ),
        "best_one_swap_cvar90_after_v277": (
            None if pd.isna(best_cvar90_after) else float(best_cvar90_after)
        ),
        "current_exposure_v277": float(row["current_exposure_v277"]),
        "current_objective_return_v277": float(row["current_objective_return_v277"]),
        "post_restricted_pool_milp_one_swap_local_optimality_cleared_v277": (
            local_optimality_cleared
        ),
        "full_universe_integer_optimality_claim_allowed_v277": False,
        "paper1_promotion_allowed_v277": False,
        "paper4_working_champion_changed_v277": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v277 is post-v276 restricted-pool MILP one-swap pricing only; "
            "multi-swap/global and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v277_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v277": status}, indent=2, sort_keys=True))
