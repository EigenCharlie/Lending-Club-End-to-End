#!/usr/bin/env python3
"""Build Paper 4 v306 applied post-v304 one-swap repair artifacts."""
...
def _best_v305_feasible_swap() -> pd.Series:
    top = read_csv("paper4_v305_post_v304_one_swap_top_candidates.csv")
    if top.empty:
        raise RuntimeError("Missing v305 top candidates; run v305 first.")
    feasible = top.loc[top[f"one_swap_improves_return_v{SIGNAL_VERSION}"].astype(bool)].copy()
    if feasible.empty:
        raise RuntimeError("v306 requires a feasible improving v305 one-swap signal.")
    return feasible.sort_values(f"return_delta_v{SIGNAL_VERSION}", ascending=False).iloc[0]


def _base_v304_solution() -> tuple[pd.DataFrame, pd.Series]:
    ...
    if solutions.empty or allocations.empty:
        raise RuntimeError("Missing v304 solution artifacts for v306.")
    best_row = solutions.sort_values(f"objective_return_v{BASE_VERSION}", ascending=False).iloc[0]
    reward = float(best_row[f"reward_per_imputation_penalty_v{BASE_VERSION}"])
    selected = allocations.loc[
        np.isclose(
            allocations[f"reward_per_imputation_penalty_v{BASE_VERSION}"].to_numpy(float),
            reward,
        )
    ].copy()
...
def _source_caps_for_base_reward() -> pd.DataFrame:
    source_summary = read_csv("paper4_v304_bounded_reward_source_summary.csv")
    if source_summary.empty:
        raise RuntimeError("Missing v304 source summary for v306.")
    source_summary["source_id"] = source_summary["source_id"].astype(str)
    return source_summary.loc[
        np.isclose(source_summary[f"reward_per_imputation_penalty_v{BASE_VERSION}"], 0.0)
    ].copy()
    ...
    *,
    ...
    for family in FAMILIES:
        ...
        for source_id in sorted(universe[family].dropna().astype(str).unique()):
            ...
            rows.append(
                {
                    f"portfolio_label_v{VERSION}": "post_v304_best_one_swap_repair_candidate",
                    ...
                    f"claim_boundary_v{VERSION}": "v306 post-swap source diagnostic only",
                }
            )
...
def _build_repair() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    selected, base_row = _base_v304_solution()
    ...
    source_caps = _source_caps_for_base_reward()
    best = _best_v305_feasible_swap()
    if universe.empty or selected.empty or v305_summary.empty or v47_panel.empty:
        raise RuntimeError("Missing v55, v304, v305 or v47 inputs for v306.")
    ...
    kept = selected.loc[
        ~selected["loan_id"].eq(drop_id), keep_cols + [f"pool_role_v{BASE_VERSION}"]
    ]
    if len(kept) != len(selected) - 1:
        raise RuntimeError(f"Could not drop selected loan {drop_id} from v304 portfolio.")
    add_row[f"pool_role_v{BASE_VERSION}"] = "not_in_v304_bounded_pool"
    ...
    repaired[f"portfolio_label_v{VERSION}"] = "post_v304_best_one_swap_repair_candidate"
    ...
    repaired[f"claim_boundary_v{VERSION}"] = (
        "post-v304 one-swap repair candidate only; requires post-repair repricing"
    )
        ...
        [
            {
                ...
                f"dropped_had_observed_proxy_v{VERSION}": drop_id in observed_ids,
                f"claim_boundary_v{VERSION}": (
                    "best v305 feasible swap applied; not post-repair optimality"
                ),
            }
        ]
    )
    ...
    budget_feasible = (
        exposure >= float(v305_row[f"exposure_min_v{SIGNAL_VERSION}"]) - 1e-7
        and exposure <= float(v305_row[f"exposure_max_v{SIGNAL_VERSION}"]) + 1e-7
    )
    cvar_feasible = cvar90 <= float(v305_row[f"current_cvar90_v{SIGNAL_VERSION}"]) + 1e-7
        ...
        [
            {
                ...
                f"next_artifact_v{VERSION}": f"paper4_v{NEXT_VERSION}_post_v306_reprice.csv",
                f"claim_boundary_v{VERSION}": (
                    "applied best v305 one-swap repair; repricing and proxy coverage gates remain"
                ),
            }
        ]
    )
...
def _claim_blockers(summary: pd.DataFrame, coverage: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {
                f"blocker_id_v{VERSION}": "best_v305_swap_repair_candidate_created",
                f"blocking_v{VERSION}": not feasible,
                f"evidence_count_v{VERSION}": int(feasible),
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v306_apply_next_post_v304_swap_summary.csv"
                ),
                f"claim_boundary_v{VERSION}": (
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": f"paper4_v{NEXT_VERSION}_post_v306_reprice.csv",
            ...
            },
            {
                ...
                f"claim_boundary_v{VERSION}": (
                    "added loan lacks observed v47/v299 cashflow proxy coverage"
                ),
            },
            {
...
def _claim_matrix() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v306_best_v305_swap_applied",
                "allowed": True,
                "artifact": "paper4_v306_apply_next_post_v304_swap_summary.csv",
                "boundary": "single feasible v305 repair applied and audited",
            },
            {
                "claim_id": "v306_return_improves_and_cvar_lowers_vs_v304",
                "allowed": True,
                "artifact": "paper4_v306_apply_next_post_v304_swap_summary.csv",
                "boundary": "static scenario proxy metrics only; requires repricing",
            },
            {
                "claim_id": "v306_post_repair_local_optimality",
                "allowed": False,
                "artifact": "paper4_v306_claim_blockers.csv",
                "boundary": "post-v306 repricing has not been executed",
            },
            {
                "claim_id": "v306_working_champion",
                "allowed": False,
                "artifact": "paper4_v306_claim_blockers.csv",
                "boundary": "global, dynamic, online and deployment evidence missing",
            },
            {
                "claim_id": "v306_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v306_claim_blockers.csv",
...
def _update_claim_boundaries() -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v306 applied post-v304 one-swap repair candidate.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v306_apply_next_post_v304_swap_summary.csv"
                ),
                "boundary": "Best feasible v305 swap applied; requires post-repair repricing.",
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v306 improves return and lowers CVaR versus v304.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v306_apply_next_post_v304_swap_summary.csv"
            ...
            {
                "claim": "v306 repaired portfolio is post-repair locally optimal.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v306_claim_blockers.csv"
                ),
                "boundary": "Post-v306 repricing has not been executed.",
            ...
            {
                "claim": "v306 authorizes a Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v306_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v306 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v306_claim_blockers.csv"
                ),
...
def _update_backlog() -> None:
        ...
        [
            {
                ...
                "executable_item": (
                    "v306 applies the best CVaR-feasible v305 one-swap signal to the "
                    "v304 best-return bounded solution and audits return, CVaR, source "
                    "caps and proxy coverage."
                ),
                "status": "post_v304_best_one_swap_applied_requires_repricing",
                "next_artifact": f"paper4_v{NEXT_VERSION}_post_v306_reprice.csv",
                "success_condition": (
                    "post-v306 repricing finds no feasible improving one-swaps or records "
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v306")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V306_APPLY_POST_V304_SWAP_START -->"
    end = "<!-- V306_APPLY_POST_V304_SWAP_END -->"
    block = f"""
{start}
...
- Added loan: `{status["added_loan_id_v306"]}`.
- Dropped loan: `{status["dropped_loan_id_v306"]}`.
- Return delta vs v304: `{status["delta_return_vs_v304_v306"]}`.
- CVaR90 delta vs v304: `{status["delta_cvar90_vs_v304_v306"]}`.
- Objective return: `{status["objective_return_v306"]}`.
- CVaR90 after repair: `{status["scenario_loss_cvar90_v306"]}`.
- Source cap violations: `{status["source_cap_violations_v306"]}`.
- Missing v47 proxy rows: `{status["missing_v47_proxy_rows_v306"]}`.
- Delta missing proxy rows vs v304:
  `{status["delta_missing_v47_proxy_rows_vs_v304_v306"]}`.
- Repair candidate feasible: `{status["repair_candidate_feasible_v306"]}`.
- Post-repair repricing required:
  `{status["post_repair_pricing_required_v306"]}`.
...
def main() -> None:
    ...
    (TABLE_DIR / "paper4_v306_apply_next_post_v304_swap_allocations.parquet").parent.mkdir(
        parents=True, exist_ok=True
    )
    allocations.to_parquet(TABLE_DIR / "paper4_v306_apply_next_post_v304_swap_allocations.parquet")
    write_csv(TABLE_DIR / "paper4_v306_apply_next_post_v304_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v306_apply_next_post_v304_swap_action.csv", action)
    write_csv(
        TABLE_DIR / "paper4_v306_apply_next_post_v304_swap_source_summary.csv", source_summary
    )
    write_csv(TABLE_DIR / "paper4_v306_proxy_coverage_delta.csv", coverage)
    write_csv(TABLE_DIR / "paper4_v306_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v306_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v306_apply_next_post_v304_swap",
        "schema_version": "2026-05-16.306",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "signal_version_v306": SIGNAL_VERSION,
        "base_version_v306": BASE_VERSION,
        "baseline_version_v306": BASELINE_VERSION,
        "summary_rows_v306": int(len(summary)),
        "action_rows_v306": int(len(action)),
        "allocation_rows_v306": int(len(allocations)),
        "source_summary_rows_v306": int(len(source_summary)),
        "coverage_rows_v306": int(len(coverage)),
        "claim_blocker_rows_v306": int(len(blockers)),
        "claim_matrix_rows_v306": int(len(claim_matrix)),
        "added_loan_id_v306": str(action_row[f"added_loan_id_v{VERSION}"]),
        "dropped_loan_id_v306": str(action_row[f"dropped_loan_id_v{VERSION}"]),
        "added_has_observed_proxy_v306": bool(action_row[f"added_has_observed_proxy_v{VERSION}"]),
        "dropped_had_observed_proxy_v306": bool(
            action_row[f"dropped_had_observed_proxy_v{VERSION}"]
        ),
        "selected_rows_v306": int(row[f"selected_rows_v{VERSION}"]),
        "base_selected_rows_v306": int(row[f"base_selected_rows_v{VERSION}"]),
        "cardinality_restored_v306": bool(row[f"cardinality_restored_v{VERSION}"]),
        "portfolio_exposure_v306": float(row[f"portfolio_exposure_v{VERSION}"]),
        "objective_return_v306": float(row[f"objective_return_v{VERSION}"]),
        "scenario_loss_mean_v306": float(row[f"scenario_loss_mean_v{VERSION}"]),
        "scenario_loss_cvar90_v306": float(row[f"scenario_loss_cvar90_v{VERSION}"]),
        "source_cap_violations_v306": int(row[f"source_cap_violations_v{VERSION}"]),
        "max_source_share_v306": float(row[f"max_source_share_v{VERSION}"]),
        "min_source_slack_v306": float(row[f"min_source_slack_v{VERSION}"]),
        "observed_v47_proxy_rows_v306": int(row[f"observed_v47_proxy_rows_v{VERSION}"]),
        "missing_v47_proxy_rows_v306": int(row[f"missing_v47_proxy_rows_v{VERSION}"]),
        "delta_missing_v47_proxy_rows_vs_v304_v306": int(
            row[f"delta_missing_v47_proxy_rows_vs_v{BASE_VERSION}_v{VERSION}"]
        ),
        "delta_return_vs_v304_v306": float(row[f"delta_return_vs_v{BASE_VERSION}_v{VERSION}"]),
        "delta_cvar90_vs_v304_v306": float(row[f"delta_cvar90_vs_v{BASE_VERSION}_v{VERSION}"]),
        "delta_exposure_vs_v304_v306": float(row[f"delta_exposure_vs_v{BASE_VERSION}_v{VERSION}"]),
        "budget_feasible_v306": bool(row[f"budget_feasible_v{VERSION}"]),
        "cvar_feasible_v306": bool(row[f"cvar_feasible_v{VERSION}"]),
        "source_feasible_v306": bool(row[f"source_feasible_v{VERSION}"]),
        "repair_candidate_feasible_v306": bool(row[f"repair_candidate_feasible_v{VERSION}"]),
        "post_repair_pricing_required_v306": bool(row[f"post_repair_pricing_required_v{VERSION}"]),
        "post_repair_one_swap_optimality_claim_allowed_v306": False,
        "working_champion_claim_allowed_v306": False,
        "full_universe_integer_optimality_claim_allowed_v306": False,
        "paper1_promotion_allowed_v306": False,
        "paper4_working_champion_changed_v306": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "next_artifact_v306": f"paper4_v{NEXT_VERSION}_post_v306_reprice.csv",
        "claim_boundary": (
            "v306 applies a post-v304 one-swap repair; no working champion or final "
            "promotion is authorized"
        ),
    }
    write_json(STATUS_DIR / "paper4_v306_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v306": status}, indent=2, sort_keys=True))
