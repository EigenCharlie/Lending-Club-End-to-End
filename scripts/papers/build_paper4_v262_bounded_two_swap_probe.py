#!/usr/bin/env python3
"""Build Paper 4 v262 bounded two-swap probe after v261 local clearance."""
...
def _build_artifacts() -> dict[str, Any]:
    ...
    selected = read_parquet("paper4_v260_next_one_swap_repair_allocations.parquet").reset_index(
        drop=True
    )
    ...
    repair_summary = read_csv("paper4_v260_next_one_swap_repair_summary.csv")
    repair_row = repair_summary.iloc[0]
    if universe.empty or selected.empty or source_summary.empty:
        raise RuntimeError("Missing universe, v260 repaired portfolio, or source caps.")
    ...
    for primary_rank, (primary_add_pos, primary_drop_pos, primary_delta, primary_net) in enumerate(
        ...
        primary_row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "bounded primary one-swap from post-v260 source-prefilter frontier; "
        ...
        for relief_candidate_pos, relief_selected_pos in np.argwhere(source_prefilter_mask):
            ...
            row: dict[str, Any] = {
                ...
                f"bounded_multi_swap_scope_v{VERSION}": (
                    f"top_{PRIMARY_FRONTIER_LIMIT}_post_v260_source_prefilter_two_swap"
                ),
                f"claim_boundary_v{VERSION}": (
        ...
        stage_rows.append(
            {
                ...
                f"claim_boundary_v{VERSION}": (
                    "bounded relief search from one post-v260 source-prefilter primary swap"
                ),
            }
        )
    ...
    summary = pd.DataFrame(
        [
            {
                f"probe_label_v{VERSION}": "bounded_two_swap_after_v261_local_clearance",
                ...
                f"claim_boundary_v{VERSION}": (
                    ...
                    "post-v260 primary swaps only; no exhaustive multi-swap/global proof"
                ),
            }
        ]
    )
    improvement_next = (
        "paper4_v263_apply_bounded_two_swap_repair.csv"
        if cvar_feasible > 0
        else "paper4_v263_broader_three_swap_or_global_gap_probe.csv"
    )
        ...
        [
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v263_broader_three_swap_or_global_gap_probe.csv"
                ),
                f"claim_boundary_v{VERSION}": (
                    "v262 searches a bounded two-swap frontier, not all multi-loan exchanges"
                ),
            },
            {
                ...
                f"blocking_v{VERSION}": True,
                f"evidence_count_v{VERSION}": 1,
                f"required_next_artifact_v{VERSION}": "paper4_v263_global_gap_certificate_protocol.csv",
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v262_bounded_two_swap_probe_executed",
                "allowed": True,
                "artifact": "paper4_v262_bounded_two_swap_summary.csv",
                "boundary": "bounded source-relief two-swap probe after v261 local clearance",
            },
            {
                "claim_id": "v262_bounded_two_swap_improvement_found",
                "allowed": cvar_feasible > 0,
                "artifact": "paper4_v262_bounded_two_swap_top_candidates.csv",
                "boundary": "existence claim only within bounded post-v260 two-swap frontier",
            },
            {
                "claim_id": "v262_multi_swap_or_global_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v262_claim_blockers.csv",
                "boundary": "bounded two-swap probe is not exhaustive/global",
            },
            {
                "claim_id": "v262_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v262_claim_blockers.csv",
...
def _update_claim_boundaries(*, improvement_found: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v262 bounded two-swap source-relief probe after v261.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v262_bounded_two_swap_summary.csv"
                ),
                "boundary": (
                    "Searches only the top bounded post-v260 source-prefilter two-swap frontier."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v262 finds bounded two-swap improvements over the v260 candidate.",
                "allowed": improvement_found,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v262_bounded_two_swap_top_candidates.csv"
                ),
                "boundary": (
                    "Existence claim only within the bounded v262 frontier; not exhaustive."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v262 proves multi-swap or global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v262_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v262 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v262_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v262 probes a bounded two-swap source-relief frontier after v261 "
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V262_BOUNDED_TWO_SWAP_PROBE_START -->"
    end = "<!-- V262_BOUNDED_TWO_SWAP_PROBE_END -->"
    best_delta = status["best_two_swap_return_delta_v262"]
    best_delta_text = (
...
frontier is explicitly limited to the top `{status["primary_frontier_limit_v262"]}`
post-v260 source-prefilter primary swaps.

### Results

- One-swap source-prefilter primary rows:
  `{status["one_swap_source_prefilter_pair_rows_v262"]}`.
- Primary frontier rows searched:
  `{status["primary_frontier_rows_v262"]}`.
- Base two-swap rows in bounded frontier:
  `{status["base_two_swap_pair_rows_v262"]}`.
- Source-prefilter two-swap rows:
  `{status["source_prefilter_two_swap_pair_rows_v262"]}`.
- Exact source-feasible two-swap rows:
  `{status["source_exact_two_swap_pair_rows_v262"]}`.
- CVaR-feasible improving two-swap rows:
  `{status["cvar_feasible_two_swap_pair_rows_v262"]}`.
...
def main() -> None:
    ...
    write_csv(
        TABLE_DIR / "paper4_v262_bounded_two_swap_primary_frontier.csv",
        artifacts["primary_frontier"],
    )
    write_csv(
        TABLE_DIR / "paper4_v262_bounded_two_swap_stage_summary.csv", artifacts["stage_summary"]
    )
    write_csv(
        TABLE_DIR / "paper4_v262_bounded_two_swap_candidates.csv", artifacts["two_swap_candidates"]
    )
    write_csv(
        TABLE_DIR / "paper4_v262_bounded_two_swap_top_candidates.csv", artifacts["top_candidates"]
    )
    write_csv(TABLE_DIR / "paper4_v262_bounded_two_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v262_claim_blockers.csv", artifacts["blockers"])
    write_csv(TABLE_DIR / "paper4_v262_claim_matrix_delta.csv", artifacts["claim_matrix"])
    ...
    status = {
        "phase": "v262_bounded_two_swap_after_v261_local_clearance",
        "schema_version": "2026-05-15.262",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "previous_repair_version_v262": PREVIOUS_REPAIR_VERSION,
        "terminal_reprice_version_v262": TERMINAL_REPRICE_VERSION,
        "primary_frontier_limit_v262": PRIMARY_FRONTIER_LIMIT,
        "primary_frontier_rows_v262": int(row["primary_frontier_rows_v262"]),
        "one_swap_return_improving_pair_rows_v262": int(
            row["one_swap_return_improving_pair_rows_v262"]
        ),
        "one_swap_budget_return_feasible_pair_rows_v262": int(
            row["one_swap_budget_return_feasible_pair_rows_v262"]
        ),
        "one_swap_source_prefilter_pair_rows_v262": int(
            row["one_swap_source_prefilter_pair_rows_v262"]
        ),
        "base_two_swap_pair_rows_v262": int(row["base_two_swap_pair_rows_v262"]),
        "source_prefilter_two_swap_pair_rows_v262": int(
            row["source_prefilter_two_swap_pair_rows_v262"]
        ),
        "source_exact_two_swap_pair_rows_v262": int(row["source_exact_two_swap_pair_rows_v262"]),
        "cvar_feasible_two_swap_pair_rows_v262": int(row["cvar_feasible_two_swap_pair_rows_v262"]),
        "top_candidate_rows_v262": int(row["top_candidate_rows_v262"]),
        "best_two_swap_return_delta_v262": (None if pd.isna(best_delta) else float(best_delta)),
        "best_two_swap_cvar90_after_v262": None if pd.isna(best_cvar) else float(best_cvar),
        "best_two_swap_objective_after_v262": (
            None if pd.isna(best_objective) else float(best_objective)
        ),
        "bounded_two_swap_improvement_found_v262": improvement_found,
        "multi_swap_integer_optimality_claim_allowed_v262": False,
        "full_universe_integer_optimality_claim_allowed_v262": False,
        "paper1_promotion_allowed_v262": False,
        "paper4_working_champion_changed_v262": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_blocker_rows_v262": int(len(artifacts["blockers"])),
        "claim_matrix_rows_v262": int(len(artifacts["claim_matrix"])),
        "claim_boundary": (
            "v262 is bounded two-swap source-relief evidence only; exhaustive "
            "multi-swap/global and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v262_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v262": status}, indent=2, sort_keys=True))
