#!/usr/bin/env python3
"""Build Paper 4 v296 post-v295 one-swap repricing artifacts."""
...
VERSION = 296
PREVIOUS_CHALLENGER_VERSION = 295
BASE_REPAIR_VERSION = 279
NEXT_VERSION = 297
TARGET_SELECTED_ROWS = 171
REPRICE_SCOPE = "post_v295_broader_pool_one_drop_one_add_whole_loan_swap"
...
def _candidate_pairs_for_reprice() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    previous_summary = read_csv("paper4_v295_broader_multi_swap_or_global_gap_probe.csv")
    if universe.empty or selected.empty or source_summary.empty or previous_summary.empty:
    ...
    policy_id = "v295_broader_post_v293_multi_swap_milp"
    regime = "post_v295_broader_pool_challenger"
    current_exposure = float(previous_row[f"portfolio_exposure_v{PREVIOUS_CHALLENGER_VERSION}"])
    exposure_min = float(previous_row[f"exposure_min_v{PREVIOUS_CHALLENGER_VERSION}"])
    exposure_max = float(previous_row[f"exposure_max_v{PREVIOUS_CHALLENGER_VERSION}"])
    cvar_cap = float(previous_row[f"scenario_loss_cvar90_v{PREVIOUS_CHALLENGER_VERSION}"])
    current_objective_return = float(
        previous_row[f"objective_return_v{PREVIOUS_CHALLENGER_VERSION}"]
    )
    ...
    for candidate_pos, selected_pos in np.argwhere(source_prefilter_mask):
        ...
        row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "post-v295 one-swap pricing only; not multi-swap or global proof"
            ),
        }
    ...
    local_claim_boundary = (
        "post-v295 one-swap screen cleared; broader multi-swap/global proof still missing"
        if improving_pairs == 0
        else "post-v295 one-swap screen found improvements; repair/reprice loop must continue"
    )
    summary = pd.DataFrame(
        [
            {
                ...
                f"post_v295_one_swap_local_optimality_cleared_v{VERSION}": (improving_pairs == 0),
    ...
    stage_summary[f"claim_boundary_v{VERSION}"] = (
        "post-v295 broader-pool one-swap screen stage count only"
    )
...
def _claim_blockers(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_apply_next_post_v295_swap.csv"
        if improving > 0
    ...
    return pd.DataFrame(
        [
            {
                f"blocker_id_v{VERSION}": "post_v295_one_swap_improvement_found",
                f"blocking_v{VERSION}": improving > 0,
                f"evidence_count_v{VERSION}": improving,
                f"required_next_artifact_v{VERSION}": next_artifact,
                f"claim_boundary_v{VERSION}": (
                    "feasible improving post-v295 one-swaps block local optimality"
                    if improving > 0
                    else "no feasible improving post-v295 one-swaps remain"
                ),
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": next_artifact,
                f"claim_boundary_v{VERSION}": (
                    "v295/v296 remain bounded-pool and one-swap evidence"
                ),
            },
            {
...
def _claim_matrix(*, local_optimality_cleared: bool) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v296_post_v295_one_swap_reprice_executed",
                "allowed": True,
                "artifact": "paper4_v296_post_v295_one_swap_summary.csv",
                "boundary": "post-v295 one-drop/one-add screen completed",
            },
            {
                "claim_id": "v296_post_v295_one_swap_local_optimality",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v296_claim_blockers.csv",
                "boundary": "allowed only within one-drop/one-add scope",
            },
            {
                "claim_id": "v296_working_champion",
                "allowed": False,
                "artifact": "paper4_v296_claim_blockers.csv",
                "boundary": "global and dynamic evidence missing",
            },
            {
                "claim_id": "v296_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v296_claim_blockers.csv",
                "boundary": "global certificate missing",
            },
            {
                "claim_id": "v296_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v296_claim_blockers.csv",
...
def _update_claim_boundaries(*, local_optimality_cleared: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v296 post-v295 one-swap repricing screen.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v296_post_v295_one_swap_summary.csv"
                ),
                "boundary": "One-drop/one-add screen after v295; not global integer proof.",
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v296 clears post-v295 one-swap local optimality.",
                "allowed": local_optimality_cleared,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v296_claim_blockers.csv"
                ),
                "boundary": "Scope-limited to one-drop/one-add swaps after v295.",
                "prohibited_claim_flag": not local_optimality_cleared,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v296 authorizes a new Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v296_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v296 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v296_claim_blockers.csv"
                ),
...
def _update_backlog(*, local_optimality_cleared: bool) -> None:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_full_universe_branch_price_bound_or_dynamic_replay.csv"
        if local_optimality_cleared
        else f"paper4_v{NEXT_VERSION}_apply_next_post_v295_swap.csv"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v296 reruns one-swap pricing after the v295 broader-pool "
                    "bounded challenger over all non-selected comparable loans."
                ),
                "status": (
                    "post_v295_one_swap_local_optimality_cleared"
                    if local_optimality_cleared
                    else "post_v295_one_swap_improvement_found"
                ),
                ...
                "last_wave": "v296",
                "execution_result": (
                    "post_v295_one_swap_reprice_cleared"
                    if local_optimality_cleared
                    else "post_v295_one_swap_reprice_found_improvements"
                ),
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v296")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V296_POST_V295_ONE_SWAP_REPRICE_START -->"
    end = "<!-- V296_POST_V295_ONE_SWAP_REPRICE_END -->"
    best_delta = status["best_one_swap_return_delta_v296"]
    best_delta_text = (
...
- Selected rows: `{status["selected_rows_v296"]}`.
- Candidate add rows: `{status["candidate_add_rows_v296"]}`.
- Pair rows screened: `{status["total_pair_rows_screened_v296"]}`.
- Return-improving pairs: `{status["return_improving_pair_rows_v296"]}`.
- Budget+return feasible pairs: `{status["budget_return_feasible_pair_rows_v296"]}`.
- Source prefilter pairs: `{status["source_prefilter_pair_rows_v296"]}`.
- Exact source-feasible pairs: `{status["source_exact_pair_rows_v296"]}`.
- CVaR-feasible improving one-swaps: `{status["one_swap_improving_rows_v296"]}`.
- Best post-v295 one-swap return delta: `{best_delta_text}`.
- Post-v295 one-swap local optimality cleared:
  `{status["post_v295_one_swap_local_optimality_cleared_v296"]}`.
...
def main() -> None:
    ...
    local_optimality_cleared = bool(
        summary[f"post_v295_one_swap_local_optimality_cleared_v{VERSION}"].iloc[0]
    )
    blockers = _claim_blockers(summary)
    claim_matrix = _claim_matrix(local_optimality_cleared=local_optimality_cleared)

    write_csv(TABLE_DIR / "paper4_v296_post_v295_one_swap_reprice.csv", pairs)
    write_csv(TABLE_DIR / "paper4_v296_post_v295_one_swap_top_candidates.csv", top_candidates)
    write_csv(TABLE_DIR / "paper4_v296_post_v295_one_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v296_post_v295_one_swap_stage_summary.csv", stage_summary)
    write_csv(TABLE_DIR / "paper4_v296_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v296_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v296_post_v295_broader_pool_one_swap_reprice",
        "schema_version": "2026-05-15.296",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "previous_challenger_version_v296": PREVIOUS_CHALLENGER_VERSION,
        "base_repair_version_v296": BASE_REPAIR_VERSION,
        "summary_rows_v296": int(len(summary)),
        "stage_summary_rows_v296": int(len(stage_summary)),
        "candidate_pair_rows_v296": int(len(pairs)),
        "top_candidate_rows_v296": int(len(top_candidates)),
        "claim_blocker_rows_v296": int(len(blockers)),
        "claim_matrix_rows_v296": int(len(claim_matrix)),
        "selected_rows_v296": int(row[f"selected_rows_v{VERSION}"]),
        "base_selected_rows_v296": int(row[f"base_selected_rows_v{VERSION}"]),
        "cardinality_restored_v296": bool(row[f"cardinality_restored_v{VERSION}"]),
        "candidate_add_rows_v296": int(row[f"candidate_add_rows_v{VERSION}"]),
        "total_pair_rows_screened_v296": int(row[f"total_pair_rows_screened_v{VERSION}"]),
        "return_improving_pair_rows_v296": int(row[f"return_improving_pair_rows_v{VERSION}"]),
        "budget_return_feasible_pair_rows_v296": int(
            row[f"budget_return_feasible_pair_rows_v{VERSION}"]
        ),
        "source_prefilter_pair_rows_v296": int(row[f"source_prefilter_pair_rows_v{VERSION}"]),
        "source_exact_pair_rows_v296": int(row[f"source_exact_pair_rows_v{VERSION}"]),
        "cvar_feasible_pair_rows_v296": int(row[f"cvar_feasible_pair_rows_v{VERSION}"]),
        "one_swap_improving_rows_v296": int(row[f"one_swap_improving_rows_v{VERSION}"]),
        "best_one_swap_return_delta_v296": (None if pd.isna(best_delta) else float(best_delta)),
        "best_one_swap_cvar90_after_v296": None if pd.isna(best_cvar) else float(best_cvar),
        "current_exposure_v296": float(row[f"current_exposure_v{VERSION}"]),
        "current_objective_return_v296": float(row[f"current_objective_return_v{VERSION}"]),
        "cvar_cap_v296": float(row[f"cvar_cap_v{VERSION}"]),
        "post_v295_one_swap_local_optimality_cleared_v296": local_optimality_cleared,
        "working_champion_claim_allowed_v296": False,
        "full_universe_integer_optimality_claim_allowed_v296": False,
        "paper1_promotion_allowed_v296": False,
        "paper4_working_champion_changed_v296": False,
        ...
        "claim_boundary": (
            "v296 is a post-v295 one-swap repricing gate; no working champion or "
            "final promotion is authorized"
        ),
    }
    write_json(STATUS_DIR / "paper4_v296_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v296": status}, indent=2, sort_keys=True))
