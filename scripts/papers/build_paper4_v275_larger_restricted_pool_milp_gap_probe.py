#!/usr/bin/env python3
"""Build Paper 4 v275 expanded restricted-pool MILP/gap probe artifacts."""
...
VERSION = 275
BASE_REPAIR_VERSION = 273
ONE_SWAP_REPRICE_VERSION = 274
BOUNDED_TWO_SWAP_VERSION = 268
PREVIOUS_MILP_VERSION = 272
CANDIDATE_POOL_LIMIT = 2500
...
def _build_pool(
    ...
    omitted = omitted.assign(mean_return_v275=mean_returns[omitted.index.to_numpy()])
    ...
    previous_pool = read_parquet("paper4_v272_restricted_pool_milp_allocations.parquet")
    previous_pool_ids = (
        set(previous_pool["loan_id"].dropna().astype(str)) if not previous_pool.empty else set()
    )
    top_ids = set(
        omitted.sort_values("mean_return_v275", ascending=False)
    ...
    selected_pool["pool_role_v275"] = "current_v273_selected"
    ...
    candidate_pool["pool_role_v275"] = np.select(
        [
        ...
        [
            "omitted_candidate_from_top2500_and_previous_pool_or_frontier",
            "omitted_candidate_from_previous_pool_or_v268_frontier",
            "omitted_candidate_from_top2500_mean_return",
        ],
        default="omitted_candidate_from_expanded_pool",
    )
    keep_cols = ["loan_id", "loan_amnt", *FAMILIES, "pool_role_v275"]
...
def _update_claim_boundaries(*, improvement_found: bool, incumbent_available: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v275 restricted-pool MILP/gap probe after v274.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v275_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Restricted to current v273 selected loans plus a top-2500 omitted candidate "
                    "pool, v272 pool loans, and v268 frontier loans; not full universe."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v275 finds a better restricted-pool MILP incumbent than v273.",
                "allowed": bool(improvement_found and incumbent_available),
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v275_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Existence claim only inside the restricted v275 pool and solver tolerance."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v275 proves global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v275_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v275 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v275_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v275 solves an expanded restricted-pool binary MILP over the v273 selected "
                    "loans plus top omitted candidates after v274 cleared one-swap local pricing."
                ),
                "status": (
                ...
                "next_artifact": (
                    "paper4_v276_apply_restricted_pool_candidate_or_decompose_swaps.csv"
                    if improvement_found
                    else "paper4_v276_full_universe_gap_certificate_protocol.csv"
                ),
                "success_condition": (
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V275_RESTRICTED_POOL_MILP_GAP_PROBE_START -->"
    end = "<!-- V275_RESTRICTED_POOL_MILP_GAP_PROBE_END -->"
    block = f"""
{start}
...
- Pool rows: `{status["pool_rows_v275"]}`.
- Current selected rows: `{status["current_selected_rows_v275"]}`.
- Omitted candidate rows: `{status["omitted_candidate_rows_v275"]}`.
- MILP incumbent available: `{status["milp_incumbent_available_v275"]}`.
- MILP success flag: `{status["milp_success_v275"]}`.
- MILP gap: `{status["milp_gap_v275"]}`.
- Objective delta vs v273:
  `{status["objective_delta_vs_v273_v275"]}`.
- CVaR90 delta vs v273:
  `{status["cvar90_delta_vs_v273_v275"]}`.
- Better restricted-pool incumbent found:
  `{status["restricted_pool_improvement_found_v275"]}`.
...
def main() -> None:
    ...
    if universe.empty or selected.empty or source_caps.empty:
        raise RuntimeError("Missing v55 universe, v273 selected portfolio, or source caps.")
    ...
    pool[f"milp_action_v{VERSION}"] = np.select(
        ...
        ["dropped_by_restricted_pool_milp", "added_by_restricted_pool_milp", "kept_from_v273"],
    ...
    summary = pd.DataFrame(
        [
            {
                f"probe_label_v{VERSION}": "restricted_pool_milp_after_v274_no_two_swap_improvement",
                ...
                f"objective_delta_vs_v273_v{VERSION}": objective_delta,
                f"portfolio_exposure_v{VERSION}": exposure,
                f"scenario_loss_cvar90_v{VERSION}": cvar90,
                f"cvar90_delta_vs_v273_v{VERSION}": cvar_delta,
    ...
    blockers = pd.DataFrame(
        [
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v276_full_universe_gap_certificate_protocol.csv"
                ),
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v275_restricted_pool_milp_probe_executed",
                "allowed": True,
                "artifact": "paper4_v275_restricted_pool_milp_summary.csv",
                "boundary": "restricted pool only",
            },
            {
                "claim_id": "v275_restricted_pool_improvement_found",
                "allowed": restricted_improvement,
                "artifact": "paper4_v275_restricted_pool_milp_summary.csv",
                "boundary": "inside v275 restricted pool and solver result only",
            },
            {
                "claim_id": "v275_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v275_claim_blockers.csv",
                "boundary": "full-universe gap certificate missing",
            },
            {
                "claim_id": "v275_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v275_claim_blockers.csv",
    ...
    pool.to_parquet(
        TABLE_DIR / "paper4_v275_restricted_pool_milp_allocations.parquet",
        index=False,
        compression="zstd",
    )
    write_csv(TABLE_DIR / "paper4_v275_restricted_pool_milp_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v275_restricted_pool_milp_source_summary.csv", source_diag)
    write_csv(TABLE_DIR / "paper4_v275_restricted_pool_milp_constraints.csv", constraints)
    write_csv(TABLE_DIR / "paper4_v275_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v275_claim_matrix_delta.csv", claim_matrix)
    ...
    row = summary.iloc[0]
    status = {
        "phase": "v275_restricted_pool_milp_gap_probe_after_v274",
        "schema_version": "2026-05-15.275",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v275": BASE_REPAIR_VERSION,
        "terminal_reprice_version_v275": ONE_SWAP_REPRICE_VERSION,
        "bounded_two_swap_version_v275": BOUNDED_TWO_SWAP_VERSION,
        "previous_restricted_pool_milp_version_v275": PREVIOUS_MILP_VERSION,
        "candidate_pool_limit_v275": CANDIDATE_POOL_LIMIT,
        "pool_rows_v275": int(row["pool_rows_v275"]),
        "current_selected_rows_v275": int(row["current_selected_rows_v275"]),
        "omitted_candidate_rows_v275": int(row["omitted_candidate_rows_v275"]),
        "selected_rows_v275": int(row["selected_rows_v275"]),
        "added_rows_v275": int(row["added_rows_v275"]),
        "dropped_rows_v275": int(row["dropped_rows_v275"]),
        "objective_return_v275": float(row["objective_return_v275"]),
        "objective_delta_vs_v273_v275": float(row["objective_delta_vs_v273_v275"]),
        "portfolio_exposure_v275": float(row["portfolio_exposure_v275"]),
        "scenario_loss_cvar90_v275": float(row["scenario_loss_cvar90_v275"]),
        "cvar90_delta_vs_v273_v275": float(row["cvar90_delta_vs_v273_v275"]),
        "source_cap_violations_v275": int(row["source_cap_violations_v275"]),
        "budget_feasible_v275": bool(row["budget_feasible_v275"]),
        "cvar_feasible_v275": bool(row["cvar_feasible_v275"]),
        "source_feasible_v275": bool(row["source_feasible_v275"]),
        "restricted_pool_improvement_found_v275": restricted_improvement,
        "milp_success_v275": bool(row["milp_success_v275"]),
        "milp_incumbent_available_v275": bool(row["milp_incumbent_available_v275"]),
        "milp_status_v275": int(row["milp_status_v275"]),
        "milp_gap_v275": float(row["milp_gap_v275"]),
        "milp_dual_bound_v275": float(row["milp_dual_bound_v275"]),
        "constraint_rows_v275": int(row["constraint_rows_v275"]),
        "claim_blocker_rows_v275": int(len(blockers)),
        "claim_matrix_rows_v275": int(len(claim_matrix)),
        "restricted_pool_global_optimality_claim_allowed_v275": False,
        "full_universe_integer_optimality_claim_allowed_v275": False,
        "paper1_promotion_allowed_v275": False,
        "paper4_working_champion_changed_v275": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v275 is a restricted-pool MILP/gap probe only; full-universe global "
            "and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v275_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v275": status}, indent=2, sort_keys=True))
