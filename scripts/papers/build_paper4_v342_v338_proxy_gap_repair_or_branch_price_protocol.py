#!/usr/bin/env python3
"""Build Paper 4 v342 v338 proxy-gap repair / branch-price protocol artifacts."""
...
NEXT_ARTIFACT = f"paper4_v{NEXT_VERSION}_v338_expanded_pool_or_dual_bound_gate.csv"
...
def _build_pool(
    *,
    ...
    pool_summary = pd.DataFrame(
        [
            {
                f"pool_id_v{VERSION}": "v338_plus_observed_omitted_candidates",
            ...
            rows.append(
                {
                    ...
                    f"claim_boundary_v{VERSION}": (
                        "v342 relaxed proxy-gap repair source diagnostic only"
                    ),
                }
            )
...
def _actions(*, pool: pd.DataFrame, selected: pd.DataFrame) -> pd.DataFrame:
    ...
    changed[f"claim_boundary_v{VERSION}"] = (
        "v342 relaxed proxy-gap repair action list; post-repair repricing still required"
    )
    return changed[
        [
            f"action_v{VERSION}",
...
def _update_claim_boundaries() -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "v342 tests strict v338-preserving proxy-gap repair.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_proxy_gap_tier_summary.csv"
                ),
            ...
            {
                "claim": "v342 documents relaxed v338 proxy-gap repair infeasibility.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_proxy_gap_tier_summary.csv"
                ),
                "boundary": (
                    "No bounded-pool relaxed repair preserves the v316 return floor "
            ...
            {
                "claim": "v342 finds a relaxed v338 proxy-gap repair allocation.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_claim_blockers.csv"
                ),
                "boundary": "Relaxed v316-return/v338-CVaR tier is infeasible.",
                "prohibited_claim_flag": True,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v342 preserves the full v338 return while repairing proxy coverage.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_claim_blockers.csv"
                ),
                "boundary": "Strict v338-return/v338-CVaR repair is infeasible in the bounded pool.",
                "prohibited_claim_flag": True,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v342 proves full-universe branch-price optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v342 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v342_claim_blockers.csv"
                ),
...
def _update_backlog() -> None:
        ...
        [
            {
                ...
                "executable_item": (
                    "v342 tests whether v338's proxy coverage loss can be repaired with "
                    "observed candidates while preserving static risk/return gates."
                ),
                "status": "strict_and_relaxed_v338_proxy_repair_infeasible",
                "next_artifact": NEXT_ARTIFACT,
                "success_condition": (
                    "expand the observed-candidate pool or build a dual-bound certificate without promoting"
                ),
                "last_wave": "v342",
                "execution_result": "strict_and_relaxed_v338_proxy_repair_infeasible",
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v342")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V342_V338_PROXY_GAP_REPAIR_START -->"
    end = "<!-- V342_V338_PROXY_GAP_REPAIR_END -->"
    block = f"""
{start}
...
- Candidate pool rows: `{status["pool_rows_v342"]}`.
- Strict v338-preserving repair feasible:
  `{status["strict_v338_preserving_repair_feasible_v342"]}`.
- Relaxed repair feasible:
  `{status["relaxed_repair_feasible_v342"]}`.
- Relaxed observed proxy rows:
  `{status["relaxed_observed_proxy_rows_v342"]}`.
- Relaxed missing proxy rows:
  `{status["relaxed_missing_proxy_rows_v342"]}`.
- Relaxed return delta vs v338:
  `{status["relaxed_delta_return_vs_v338_v342"]}`.
- Relaxed CVaR delta vs v338:
  `{status["relaxed_delta_cvar90_vs_v338_v342"]}`.
- Post-v342 repricing required:
  `{status["post_v342_repricing_required_v342"]}`.
...
def main() -> None:
    ...
    if any(df.empty for df in [universe, v338, v316_summary, v338_summary, source_caps, v47_panel]):
        raise RuntimeError("Missing v342 proxy-gap repair inputs.")
    if int(v341_status["imputed_proxy_loan_rows_v341"]) != 74:
        raise RuntimeError("v342 expects the v341 v338 proxy gap to be documented.")
    ...
    main_summary = pd.DataFrame(
        [
            {
                f"gate_id_v{VERSION}": "v342_v338_proxy_gap_repair_or_branch_price_protocol",
                ...
                f"post_v342_repricing_required_v{VERSION}": relaxed_feasible,
                ...
                f"claim_boundary_v{VERSION}": (
                    "bounded proxy-gap repair protocol; strict and relaxed v338 repair tiers "
                    "are infeasible"
                ),
            }
        ]
        ...
        [
            {
                ...
                f"claim_boundary_v{VERSION}": "strict v338-return/v338-CVaR repair infeasible",
            },
            {
                ...
                f"claim_boundary_v{VERSION}": (
                    "no bounded-pool relaxed repair meets the v316 return floor under v338 CVaR"
                ),
            },
            {
                f"blocker_id_v{VERSION}": "post_v342_reprice_missing",
            ...
            },
            {
                ...
                f"claim_boundary_v{VERSION}": "v342 prices a bounded observed-candidate pool",
            },
            {
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v342_strict_v338_proxy_gap_repair_found",
                "allowed": strict_feasible,
                "artifact": "paper4_v342_proxy_gap_tier_summary.csv",
                "boundary": "strict bounded-pool feasibility only",
            },
            {
                "claim_id": "v342_relaxed_proxy_gap_repair_found",
                "allowed": relaxed_feasible,
                "artifact": "paper4_v342_relaxed_proxy_gap_allocations.parquet",
                "boundary": "relaxed v316-return/v338-CVaR candidate only",
            },
            {
                "claim_id": "v342_relaxed_observed_proxy_rows_reach_100",
                "allowed": relaxed_observed >= 100,
                "artifact": "paper4_v342_proxy_gap_tier_summary.csv",
                "boundary": "proxy coverage diagnostic only",
            },
            {
                "claim_id": "v342_preserves_full_v338_return",
                "allowed": False,
                "artifact": "paper4_v342_claim_blockers.csv",
                "boundary": "strict tier is infeasible and no relaxed repair allocation exists",
            },
            {
                "claim_id": "v342_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v342_claim_blockers.csv",
                "boundary": "bounded observed-candidate pool only",
            },
            {
                "claim_id": "v342_working_champion_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v342_claim_blockers.csv",
    ...
    write_csv(
        TABLE_DIR / "paper4_v342_v338_proxy_gap_repair_or_branch_price_protocol.csv",
        main_summary,
    )
    write_csv(TABLE_DIR / "paper4_v342_proxy_gap_pool_summary.csv", pool_summary)
    write_csv(TABLE_DIR / "paper4_v342_proxy_gap_tier_summary.csv", tier_summary)
    relaxed.to_parquet(TABLE_DIR / "paper4_v342_relaxed_proxy_gap_allocations.parquet", index=False)
    write_csv(TABLE_DIR / "paper4_v342_relaxed_proxy_gap_actions.csv", actions)
    write_csv(TABLE_DIR / "paper4_v342_relaxed_proxy_gap_source_summary.csv", source_summary)
    write_csv(TABLE_DIR / "paper4_v342_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v342_claim_matrix_delta.csv", claim_matrix)
    _update_claim_boundaries()
    _update_backlog()

    row = main_summary.iloc[0]
    status = {
        "phase": "v342_v338_proxy_gap_repair_or_branch_price_protocol",
        "schema_version": "2026-05-16.342",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_version_v342": BASE_VERSION,
        "reference_version_v342": REFERENCE_VERSION,
        "gate_version_v342": GATE_VERSION,
        "pool_rows_v342": int(row[f"pool_rows_v{VERSION}"]),
        "observed_omitted_candidate_rows_v342": int(
            row[f"observed_omitted_candidate_rows_v{VERSION}"]
        ),
        "candidate_pool_limit_per_period_v342": CANDIDATE_POOL_LIMIT_PER_PERIOD,
        "strict_v338_preserving_repair_feasible_v342": strict_feasible,
        "strict_milp_status_v342": int(strict_row[f"milp_status_v{VERSION}"]),
        "relaxed_repair_feasible_v342": relaxed_feasible,
        "relaxed_selected_rows_v342": int(row[f"relaxed_selected_rows_v{VERSION}"]),
        "relaxed_added_rows_v342": int(row[f"relaxed_added_rows_v{VERSION}"]),
        "relaxed_dropped_rows_v342": int(row[f"relaxed_dropped_rows_v{VERSION}"]),
        "relaxed_observed_proxy_rows_v342": relaxed_observed,
        "relaxed_missing_proxy_rows_v342": relaxed_missing,
        "relaxed_observed_delta_vs_v338_v342": int(
            row[f"relaxed_observed_delta_vs_v338_v{VERSION}"]
        ),
        "relaxed_missing_delta_vs_v338_v342": int(row[f"relaxed_missing_delta_vs_v338_v{VERSION}"]),
        "relaxed_objective_return_v342": _safe_float(row[f"relaxed_objective_return_v{VERSION}"]),
        "relaxed_delta_return_vs_v338_v342": relaxed_return_delta,
        "relaxed_delta_return_vs_v316_v342": _safe_float(
            row[f"relaxed_delta_return_vs_v316_v{VERSION}"]
        ),
        "relaxed_cvar90_v342": _safe_float(row[f"relaxed_cvar90_v{VERSION}"]),
        "relaxed_delta_cvar90_vs_v338_v342": relaxed_cvar_delta,
        "relaxed_delta_cvar90_vs_v316_v342": _safe_float(
            row[f"relaxed_delta_cvar90_vs_v316_v{VERSION}"]
        ),
        "source_cap_violations_v342": source_cap_violations,
        "min_source_slack_v342": min_source_slack,
        "post_v342_repricing_required_v342": relaxed_feasible,
        "strict_repair_or_champion_claim_allowed_v342": False,
        "full_universe_integer_optimality_claim_allowed_v342": False,
        "working_champion_claim_allowed_v342": False,
        "paper1_promotion_allowed_v342": False,
        "paper4_working_champion_changed_v342": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_blocker_rows_v342": int(len(blockers)),
        "claim_matrix_rows_v342": int(len(claim_matrix)),
        "next_artifact_v342": row[f"next_artifact_v{VERSION}"],
        "claim_boundary": (
            "v342 documents a bounded proxy-coverage frontier: strict v338-preserving repair "
            "and the relaxed v316-return/v338-CVaR repair are both infeasible"
        ),
    }
    write_json(STATUS_DIR / "paper4_v342_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v342": status}, indent=2, sort_keys=True))
