#!/usr/bin/env python3
"""Build Paper 4 v281 expanded restricted-pool MILP/gap probe artifacts."""
...
ONE_SWAP_REPRICE_VERSION = 280
BOUNDED_TWO_SWAP_VERSION = 268
PREVIOUS_MILP_VERSION = 278
...
def _build_pool(
    ...
    omitted = omitted.assign(mean_return_v281=mean_returns[omitted.index.to_numpy()])
    ...
    previous_pool = read_parquet("paper4_v278_restricted_pool_milp_allocations.parquet")
    previous_pool_ids = (
        set(previous_pool["loan_id"].dropna().astype(str)) if not previous_pool.empty else set()
    )
    top_ids = set(
        omitted.sort_values("mean_return_v281", ascending=False)
    ...
    selected_pool["pool_role_v281"] = "current_v279_selected"
    ...
    candidate_pool["pool_role_v281"] = np.select(
        [
        ...
        [
            "omitted_candidate_from_top5000_and_previous_pool_or_frontier",
            "omitted_candidate_from_previous_pool_or_v268_frontier",
            "omitted_candidate_from_top5000_mean_return",
        ],
        default="omitted_candidate_from_expanded_pool",
    )
    keep_cols = ["loan_id", "loan_amnt", *FAMILIES, "pool_role_v281"]
...
def _update_claim_boundaries(*, improvement_found: bool, incumbent_available: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v281 restricted-pool MILP/gap probe after v280.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v281_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Restricted to current v279 selected loans plus a top-5000 omitted candidate "
                    "pool, v278 pool loans, and v268 frontier loans; not full universe."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v281 finds a better restricted-pool MILP incumbent than v279.",
                "allowed": bool(improvement_found and incumbent_available),
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v281_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Existence claim only inside the restricted v281 pool and solver tolerance."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v281 proves global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v281_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v281 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v281_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v281 solves an expanded restricted-pool binary MILP over the v279 selected "
                    "loans plus top omitted candidates after v280 cleared one-swap local pricing."
                ),
                "status": (
                ...
                "next_artifact": (
                    "paper4_v282_apply_restricted_pool_candidate_or_decompose_swaps.csv"
                    if improvement_found
                    else "paper4_v282_full_universe_gap_certificate_protocol.csv"
                ),
                "success_condition": (
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V281_RESTRICTED_POOL_MILP_GAP_PROBE_START -->"
    end = "<!-- V281_RESTRICTED_POOL_MILP_GAP_PROBE_END -->"
    block = f"""
{start}
...
- Pool rows: `{status["pool_rows_v281"]}`.
- Current selected rows: `{status["current_selected_rows_v281"]}`.
- Omitted candidate rows: `{status["omitted_candidate_rows_v281"]}`.
- MILP incumbent available: `{status["milp_incumbent_available_v281"]}`.
- MILP success flag: `{status["milp_success_v281"]}`.
- MILP gap: `{status["milp_gap_v281"]}`.
- Objective delta vs v279:
  `{status["objective_delta_vs_v279_v281"]}`.
- CVaR90 delta vs v279:
  `{status["cvar90_delta_vs_v279_v281"]}`.
- Better restricted-pool incumbent found:
  `{status["restricted_pool_improvement_found_v281"]}`.
...
def main() -> None:
    ...
    if universe.empty or selected.empty or source_caps.empty:
        raise RuntimeError("Missing v55 universe, v279 selected portfolio, or source caps.")
    ...
    pool[f"milp_action_v{VERSION}"] = np.select(
        ...
        ["dropped_by_restricted_pool_milp", "added_by_restricted_pool_milp", "kept_from_v279"],
    ...
    summary = pd.DataFrame(
        [
            {
                f"probe_label_v{VERSION}": "restricted_pool_milp_after_v280_no_two_swap_improvement",
                ...
                f"objective_delta_vs_v279_v{VERSION}": objective_delta,
                f"portfolio_exposure_v{VERSION}": exposure,
                f"scenario_loss_cvar90_v{VERSION}": cvar90,
                f"cvar90_delta_vs_v279_v{VERSION}": cvar_delta,
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v281_restricted_pool_milp_probe_executed",
                "allowed": True,
                "artifact": "paper4_v281_restricted_pool_milp_summary.csv",
                "boundary": "restricted pool only",
            },
            {
                "claim_id": "v281_restricted_pool_improvement_found",
                "allowed": restricted_improvement,
                "artifact": "paper4_v281_restricted_pool_milp_summary.csv",
                "boundary": "inside v281 restricted pool and solver result only",
            },
            {
                "claim_id": "v281_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v281_claim_blockers.csv",
                "boundary": "full-universe gap certificate missing",
            },
            {
                "claim_id": "v281_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v281_claim_blockers.csv",
    ...
    pool.to_parquet(
        TABLE_DIR / "paper4_v281_restricted_pool_milp_allocations.parquet",
        index=False,
        compression="zstd",
    )
    write_csv(TABLE_DIR / "paper4_v281_restricted_pool_milp_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v281_restricted_pool_milp_source_summary.csv", source_diag)
    write_csv(TABLE_DIR / "paper4_v281_restricted_pool_milp_constraints.csv", constraints)
    write_csv(TABLE_DIR / "paper4_v281_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v281_claim_matrix_delta.csv", claim_matrix)
    ...
    row = summary.iloc[0]
    status = {
        "phase": "v281_restricted_pool_milp_gap_probe_after_v280",
        "schema_version": "2026-05-15.281",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v281": BASE_REPAIR_VERSION,
        "terminal_reprice_version_v281": ONE_SWAP_REPRICE_VERSION,
        "bounded_two_swap_version_v281": BOUNDED_TWO_SWAP_VERSION,
        "previous_restricted_pool_milp_version_v281": PREVIOUS_MILP_VERSION,
        "candidate_pool_limit_v281": CANDIDATE_POOL_LIMIT,
        "pool_rows_v281": int(row["pool_rows_v281"]),
        "current_selected_rows_v281": int(row["current_selected_rows_v281"]),
        "omitted_candidate_rows_v281": int(row["omitted_candidate_rows_v281"]),
        "selected_rows_v281": int(row["selected_rows_v281"]),
        "added_rows_v281": int(row["added_rows_v281"]),
        "dropped_rows_v281": int(row["dropped_rows_v281"]),
        "objective_return_v281": float(row["objective_return_v281"]),
        "objective_delta_vs_v279_v281": float(row["objective_delta_vs_v279_v281"]),
        "portfolio_exposure_v281": float(row["portfolio_exposure_v281"]),
        "scenario_loss_cvar90_v281": float(row["scenario_loss_cvar90_v281"]),
        "cvar90_delta_vs_v279_v281": float(row["cvar90_delta_vs_v279_v281"]),
        "source_cap_violations_v281": int(row["source_cap_violations_v281"]),
        "budget_feasible_v281": bool(row["budget_feasible_v281"]),
        "cvar_feasible_v281": bool(row["cvar_feasible_v281"]),
        "source_feasible_v281": bool(row["source_feasible_v281"]),
        "restricted_pool_improvement_found_v281": restricted_improvement,
        "milp_success_v281": bool(row["milp_success_v281"]),
        "milp_incumbent_available_v281": bool(row["milp_incumbent_available_v281"]),
        "milp_status_v281": int(row["milp_status_v281"]),
        "milp_gap_v281": float(row["milp_gap_v281"]),
        "milp_dual_bound_v281": float(row["milp_dual_bound_v281"]),
        "constraint_rows_v281": int(row["constraint_rows_v281"]),
        "claim_blocker_rows_v281": int(len(blockers)),
        "claim_matrix_rows_v281": int(len(claim_matrix)),
        "restricted_pool_global_optimality_claim_allowed_v281": False,
        "full_universe_integer_optimality_claim_allowed_v281": False,
        "paper1_promotion_allowed_v281": False,
        "paper4_working_champion_changed_v281": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v281 is a restricted-pool MILP/gap probe only; full-universe global "
            "and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v281_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v281": status}, indent=2, sort_keys=True))
