#!/usr/bin/env python3
"""Build Paper 4 v276 restricted-pool MILP repair candidate artifacts."""
...
MILP_PROBE_VERSION = 275
NEXT_REPRICE_VERSION = 277
...
def _update_claim_boundaries() -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v276 restricted-pool MILP repair candidate.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v276_restricted_pool_milp_repair_summary.csv"
                ),
                "boundary": (
                    "Applies the v275 restricted-pool MILP incumbent only; requires "
            ...
            {
                "claim": "v276 repaired portfolio is post-repair locally optimal.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v276_claim_blockers.csv"
                ),
                "boundary": "Post-repair pricing has not been rerun after v276.",
                "prohibited_claim_flag": True,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v276 proves full-universe global integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v276_claim_blockers.csv"
                ),
                "boundary": "v275/v276 are restricted-pool artifacts; no global gap certificate.",
                "prohibited_claim_flag": True,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v276 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v276_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v276 applies the v275 restricted-pool MILP incumbent as a repair "
                    "candidate and records the resulting portfolio metrics."
                ),
                "status": "post_restricted_pool_milp_repair_pricing_required",
                "next_artifact": "paper4_v277_post_restricted_pool_milp_reprice.csv",
                "success_condition": (
                    "post-v276 one-swap/two-swap pricing finds no feasible improving exchanges"
                ),
                "last_wave": "v276",
...
def _update_notebook(status: dict[str, object]) -> None:
    start = "<!-- V276_RESTRICTED_POOL_MILP_REPAIR_START -->"
    end = "<!-- V276_RESTRICTED_POOL_MILP_REPAIR_END -->"
    block = f"""
{start}
...
- Added loans: `{status["added_rows_v276"]}`.
- Dropped loans: `{status["dropped_rows_v276"]}`.
- Selected rows after repair: `{status["selected_rows_v276"]}`.
- Return delta vs v273:
  `{status["delta_return_vs_v273_v276"]}`.
- CVaR90 delta vs v273:
  `{status["delta_cvar90_vs_v273_v276"]}`.
- Budget feasible: `{status["budget_feasible_v276"]}`.
- Source feasible: `{status["source_feasible_v276"]}`.
- CVaR feasible: `{status["cvar_feasible_v276"]}`.
...
def main() -> None:
    ...
    milp_pool = read_parquet("paper4_v275_restricted_pool_milp_allocations.parquet")
    base_summary = read_csv("paper4_v273_restricted_pool_milp_repair_summary.csv")
    ...
    if universe.empty or milp_pool.empty or source_caps.empty:
        raise RuntimeError("Missing v55 universe, v275 MILP pool, or source caps.")

    selected = milp_pool.loc[milp_pool["selected_v275"].eq(1)].copy()
    if selected.empty:
        raise RuntimeError("v275 did not produce a selected restricted-pool incumbent.")
    ...
    selected[f"repair_action_v{VERSION}"] = selected["milp_action_v275"]
    ...
    added = selected.loc[selected["milp_action_v275"].eq("added_by_restricted_pool_milp")].copy()
    dropped = milp_pool.loc[
        milp_pool["milp_action_v275"].eq("dropped_by_restricted_pool_milp")
    ].copy()

    action = pd.concat(
        [
            added.assign(action_v276="added_by_restricted_pool_milp"),
            dropped.assign(action_v276="dropped_by_restricted_pool_milp"),
        ],
        ignore_index=True,
    )
    action = action[["action_v276", "loan_id", "loan_amnt", "mean_return_v275", *FAMILIES]].rename(
        columns={"mean_return_v275": "mean_return_v276"}
    )
    ...
    blockers = pd.DataFrame(
        [
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v277_post_restricted_pool_milp_reprice.csv"
                ),
                f"claim_boundary_v{VERSION}": "repair must be re-priced after v276",
            },
            {
                ...
                f"evidence_count_v{VERSION}": 1,
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v277_full_universe_gap_certificate_protocol.csv"
                ),
                f"claim_boundary_v{VERSION}": (
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v276_restricted_pool_milp_repair_executed",
                "allowed": True,
                "artifact": "paper4_v276_restricted_pool_milp_repair_summary.csv",
                "boundary": "v275 restricted-pool incumbent applied",
            },
            {
                "claim_id": "v276_repair_candidate_feasible",
                "allowed": feasible,
                "artifact": "paper4_v276_restricted_pool_milp_repair_summary.csv",
                "boundary": "budget/source/CVaR feasibility only",
            },
            {
                "claim_id": "v276_post_repair_local_optimality",
                "allowed": False,
                "artifact": "paper4_v276_claim_blockers.csv",
                "boundary": "post-repair pricing missing",
            },
            {
                "claim_id": "v276_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v276_claim_blockers.csv",
                "boundary": "full-universe gap certificate missing",
            },
            {
                "claim_id": "v276_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v276_claim_blockers.csv",
    ...
    selected_out.to_parquet(
        TABLE_DIR / "paper4_v276_restricted_pool_milp_repair_allocations.parquet",
        index=False,
        compression="zstd",
    )
    write_csv(TABLE_DIR / "paper4_v276_restricted_pool_milp_repair_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v276_restricted_pool_milp_repair_action.csv", action)
    write_csv(
        TABLE_DIR / "paper4_v276_restricted_pool_milp_repair_source_summary.csv",
        source_summary,
    )
    write_csv(TABLE_DIR / "paper4_v276_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v276_claim_matrix_delta.csv", claim_matrix)
    _update_claim_boundaries()
    _update_backlog()

    summary_row = summary.iloc[0]
    status = {
        "phase": "v276_restricted_pool_milp_repair",
        "schema_version": "2026-05-15.276",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "source_probe_version_v276": MILP_PROBE_VERSION,
        "allocation_rows_v276": int(len(selected_out)),
        "summary_rows_v276": int(len(summary)),
        "action_rows_v276": int(len(action)),
        "source_summary_rows_v276": int(len(source_summary)),
        "claim_blocker_rows_v276": int(len(blockers)),
        "claim_matrix_rows_v276": int(len(claim_matrix)),
        "selected_rows_v276": int(summary_row["selected_rows_v276"]),
        "added_rows_v276": int(summary_row["added_rows_v276"]),
        "dropped_rows_v276": int(summary_row["dropped_rows_v276"]),
        "portfolio_exposure_v276": float(summary_row["portfolio_exposure_v276"]),
        "objective_return_v276": float(summary_row["objective_return_v276"]),
        "scenario_loss_cvar90_v276": float(summary_row["scenario_loss_cvar90_v276"]),
        "source_cap_violations_v276": int(summary_row["source_cap_violations_v276"]),
        "delta_return_vs_v273_v276": float(summary_row["delta_return_vs_v273_v276"]),
        "delta_cvar90_vs_v273_v276": float(summary_row["delta_cvar90_vs_v273_v276"]),
        "delta_exposure_vs_v273_v276": float(summary_row["delta_exposure_vs_v273_v276"]),
        "budget_feasible_v276": bool(summary_row["budget_feasible_v276"]),
        "source_feasible_v276": bool(summary_row["source_feasible_v276"]),
        "cvar_feasible_v276": bool(summary_row["cvar_feasible_v276"]),
        "repair_candidate_feasible_v276": bool(summary_row["repair_candidate_feasible_v276"]),
        "post_repair_local_optimality_claim_allowed_v276": False,
        "restricted_pool_global_optimality_claim_allowed_v276": False,
        "full_universe_integer_optimality_claim_allowed_v276": False,
        "paper1_promotion_allowed_v276": False,
        "paper4_working_champion_changed_v276": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v276 creates a restricted-pool MILP repair candidate only; post-repair "
            "pricing and global integer claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v276_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v276": status}, indent=2, sort_keys=True))
