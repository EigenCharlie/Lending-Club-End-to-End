#!/usr/bin/env python3
"""Build Paper 4 v331 post-v330 one-swap repricing artifacts."""
...
VERSION = 331
BASE_REPAIR_VERSION = 330
PREVIOUS_REPRICE_VERSION = 330
NEXT_VERSION = 332
TARGET_SELECTED_ROWS = 171
REPRICE_SCOPE = "post_v330_candidate_one_drop_one_add_whole_loan_swap"
...
def _candidate_pairs_for_reprice() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    selected = read_parquet("paper4_v330_post_v328_swap_allocations.parquet")
    source_summary = read_csv("paper4_v330_post_v328_swap_source_summary.csv")
    ...
    if any(df.empty for df in [universe, selected, source_summary, v330_summary, v47_panel]):
    ...
    policy_id = "v330_post_v328_best_feasible_swap_candidate"
    regime = "post_v330_best_feasible_swap_candidate"
    v330_row = v330_summary.iloc[0]
    current_exposure = float(selected["loan_amnt"].sum())
    exposure_min = 842292.375
    exposure_max = 850000.0
    cvar_cap = float(v330_row[f"scenario_loss_cvar90_v{BASE_REPAIR_VERSION}"])
    current_objective_return = float(v330_row[f"objective_return_v{BASE_REPAIR_VERSION}"])
    ...
    for candidate_pos, selected_pos in np.argwhere(source_prefilter_mask):
        ...
        row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "post-v330 one-swap pricing only; not multi-swap or global proof"
            ),
    ...
    local_claim_boundary = (
        "post-v330 candidate one-swap screen cleared; dynamic/global gates still missing"
        if improving_pairs == 0
        else (
            "post-v330 candidate one-swap screen found improvements; "
        ...
        [
            {
                ...
                f"post_v330_one_swap_local_optimality_cleared_v{VERSION}": (improving_pairs == 0),
    ...
    stage_summary[f"claim_boundary_v{VERSION}"] = (
        "post-v330 candidate one-swap screen stage count only"
    )
...
def _claim_blockers(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_apply_next_post_v330_swap.csv"
        if improving > 0
        else f"paper4_v{NEXT_VERSION}_dynamic_or_dual_bound_after_v330.csv"
    )
    return pd.DataFrame(
        [
            {
                f"blocker_id_v{VERSION}": "post_v330_one_swap_improvement_found",
                f"blocking_v{VERSION}": improving > 0,
                f"evidence_count_v{VERSION}": improving,
                f"required_next_artifact_v{VERSION}": next_artifact,
                f"claim_boundary_v{VERSION}": (
                    "feasible improving post-v330 one-swaps block local optimality"
                    if improving > 0
                    else "no feasible improving post-v330 one-swaps remain"
                ),
            },
            {
                f"blocker_id_v{VERSION}": "post_v330_repair_not_working_champion",
            ...
            },
            {
                ...
                f"claim_boundary_v{VERSION}": ("v330 candidate still has missing v47 proxy rows"),
            },
            {
...
def _claim_matrix(*, local_optimality_cleared: bool) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v331_post_v330_one_swap_reprice_executed",
                "allowed": True,
                "artifact": "paper4_v331_post_v330_one_swap_summary.csv",
                "boundary": "post-v330 one-drop/one-add screen completed",
            },
            {
                "claim_id": "v331_post_v330_one_swap_local_optimality",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v331_claim_blockers.csv",
                "boundary": "allowed only within one-drop/one-add scope",
            },
            {
                "claim_id": "v331_dynamic_gate_ready",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v331_post_v330_one_swap_summary.csv",
                "boundary": "ready only to attempt proxy dynamic/global gate, not to promote",
            },
            {
                "claim_id": "v331_working_champion",
                "allowed": False,
                "artifact": "paper4_v331_claim_blockers.csv",
                "boundary": "global, dynamic, online and deployment evidence missing",
            },
            {
                "claim_id": "v331_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v331_claim_blockers.csv",
...
def _update_claim_boundaries(*, local_optimality_cleared: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v331 post-v330 one-swap repricing gate.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v331_post_v330_one_swap_summary.csv"
                ),
                "boundary": "One-drop/one-add screen after the v330 candidate.",
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v331 clears post-v330 one-swap local optimality.",
                "allowed": local_optimality_cleared,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v331_claim_blockers.csv"
                ),
                "boundary": "Scope-limited to one-drop/one-add swaps after v330.",
                "prohibited_claim_flag": not local_optimality_cleared,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v331 authorizes a Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v331_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v331 proves full-universe global integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v331_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v331 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v331_claim_blockers.csv"
                ),
...
def _update_backlog(*, local_optimality_cleared: bool) -> None:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_dynamic_or_dual_bound_after_v330.csv"
        if local_optimality_cleared
        else f"paper4_v{NEXT_VERSION}_apply_next_post_v330_swap.csv"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "executable_item": (
                    "v331 reprices the v330 applied-swap candidate against all non-selected "
                    "comparable loans with exact budget/source/CVaR screens."
                ),
                "status": (
                    "post_v330_one_swap_local_optimality_cleared"
                    if local_optimality_cleared
                    else "post_v330_one_swap_improvement_found"
                ),
                "next_artifact": next_artifact,
                "success_condition": (
                    "if cleared, run a dynamic/global gate; if not, apply the next "
                    "post-v330 repair and reprice again"
                ),
                "last_wave": "v331",
                "execution_result": (
                    "post_v330_one_swap_reprice_cleared"
                    if local_optimality_cleared
                    else "post_v330_one_swap_reprice_found_improvements"
                ),
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v331")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V331_POST_V330_ONE_SWAP_REPRICE_START -->"
    end = "<!-- V331_POST_V330_ONE_SWAP_REPRICE_END -->"
    best_delta = status["best_one_swap_return_delta_v331"]
    best_delta_text = (
        "not applicable; no source-exact return-improving swaps"
        if best_delta is None
        else str(best_delta)
    )
    best_feasible = status["best_feasible_one_swap_return_delta_v331"]
    best_feasible_text = (
...
- Selected rows: `{status["selected_rows_v331"]}`.
- Candidate add rows: `{status["candidate_add_rows_v331"]}`.
- Pair rows screened: `{status["total_pair_rows_screened_v331"]}`.
- Return-improving pairs: `{status["return_improving_pair_rows_v331"]}`.
- Budget+return feasible pairs: `{status["budget_return_feasible_pair_rows_v331"]}`.
- Source prefilter pairs: `{status["source_prefilter_pair_rows_v331"]}`.
- Exact source-feasible pairs: `{status["source_exact_pair_rows_v331"]}`.
- CVaR-feasible improving one-swaps: `{status["one_swap_improving_rows_v331"]}`.
...
- Current missing v47 proxy rows:
  `{status["current_missing_v47_proxy_rows_v331"]}`.
- Best feasible delta missing proxy rows:
  `{status["best_feasible_delta_missing_v47_proxy_rows_v331"]}`.
- Post-v330 one-swap local optimality cleared:
  `{status["post_v330_one_swap_local_optimality_cleared_v331"]}`.
- Dynamic/global gate ready:
  `{status["dynamic_gate_ready_v331"]}`.
...
def main() -> None:
    ...
    local_optimality_cleared = bool(
        summary[f"post_v330_one_swap_local_optimality_cleared_v{VERSION}"].iloc[0]
    )
    blockers = _claim_blockers(summary)
    claim_matrix = _claim_matrix(local_optimality_cleared=local_optimality_cleared)

    write_csv(TABLE_DIR / "paper4_v331_post_v330_one_swap_reprice.csv", pairs)
    write_csv(TABLE_DIR / "paper4_v331_post_v330_one_swap_top_candidates.csv", top_candidates)
    write_csv(TABLE_DIR / "paper4_v331_post_v330_one_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v331_post_v330_one_swap_stage_summary.csv", stage_summary)
    write_csv(TABLE_DIR / "paper4_v331_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v331_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v331_post_v330_reprice",
        "schema_version": "2026-05-16.331",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v331": BASE_REPAIR_VERSION,
        "previous_reprice_version_v331": PREVIOUS_REPRICE_VERSION,
        "summary_rows_v331": int(len(summary)),
        "stage_summary_rows_v331": int(len(stage_summary)),
        "candidate_pair_rows_v331": int(len(pairs)),
        "top_candidate_rows_v331": int(len(top_candidates)),
        "claim_blocker_rows_v331": int(len(blockers)),
        "claim_matrix_rows_v331": int(len(claim_matrix)),
        "selected_rows_v331": int(row[f"selected_rows_v{VERSION}"]),
        "base_selected_rows_v331": int(row[f"base_selected_rows_v{VERSION}"]),
        "cardinality_restored_v331": bool(row[f"cardinality_restored_v{VERSION}"]),
        "candidate_add_rows_v331": int(row[f"candidate_add_rows_v{VERSION}"]),
        "total_pair_rows_screened_v331": int(row[f"total_pair_rows_screened_v{VERSION}"]),
        "return_improving_pair_rows_v331": int(row[f"return_improving_pair_rows_v{VERSION}"]),
        "budget_return_feasible_pair_rows_v331": int(
            row[f"budget_return_feasible_pair_rows_v{VERSION}"]
        ),
        "source_prefilter_pair_rows_v331": int(row[f"source_prefilter_pair_rows_v{VERSION}"]),
        "source_exact_pair_rows_v331": int(row[f"source_exact_pair_rows_v{VERSION}"]),
        "cvar_feasible_pair_rows_v331": int(row[f"cvar_feasible_pair_rows_v{VERSION}"]),
        "one_swap_improving_rows_v331": int(row[f"one_swap_improving_rows_v{VERSION}"]),
        "best_one_swap_return_delta_v331": _float_or_none(best_delta),
        "best_one_swap_cvar90_after_v331": _float_or_none(best_cvar),
        "best_feasible_one_swap_return_delta_v331": _float_or_none(best_feasible_delta),
        "best_feasible_one_swap_cvar90_after_v331": _float_or_none(best_feasible_cvar),
        "current_missing_v47_proxy_rows_v331": int(
            row[f"current_missing_v47_proxy_rows_v{VERSION}"]
        ),
        "best_feasible_delta_missing_v47_proxy_rows_v331": int(
            row[f"best_feasible_delta_missing_v47_proxy_rows_v{VERSION}"]
        ),
        "current_exposure_v331": float(row[f"current_exposure_v{VERSION}"]),
        "current_objective_return_v331": float(row[f"current_objective_return_v{VERSION}"]),
        "current_loss_mean_v331": float(row[f"current_loss_mean_v{VERSION}"]),
        "current_cvar90_v331": float(row[f"current_cvar90_v{VERSION}"]),
        "post_v330_one_swap_local_optimality_cleared_v331": local_optimality_cleared,
        "dynamic_gate_ready_v331": local_optimality_cleared,
        "working_champion_claim_allowed_v331": False,
        "full_universe_integer_optimality_claim_allowed_v331": False,
        "paper1_promotion_allowed_v331": False,
        "paper4_working_champion_changed_v331": False,
        ...
        "claim_boundary": (
            "v331 is a post-v330 one-swap repricing gate; no working champion or "
            "final promotion is authorized"
        ),
    }
    write_json(STATUS_DIR / "paper4_v331_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v331": status}, indent=2, sort_keys=True))
