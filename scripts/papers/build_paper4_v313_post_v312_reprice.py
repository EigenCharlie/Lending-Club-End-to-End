#!/usr/bin/env python3
"""Build Paper 4 v313 post-v312 one-swap repricing artifacts."""
...
VERSION = 313
BASE_REPAIR_VERSION = 312
PREVIOUS_REPRICE_VERSION = 311
NEXT_VERSION = 314
TARGET_SELECTED_ROWS = 171
REPRICE_SCOPE = "post_v312_repair_one_drop_one_add_whole_loan_swap"
...
def _candidate_pairs_for_reprice() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    selected = read_parquet("paper4_v312_apply_next_post_v310_swap_allocations.parquet")
    source_summary = read_csv("paper4_v312_apply_next_post_v310_swap_source_summary.csv")
    v312_summary = read_csv("paper4_v312_apply_next_post_v310_swap_summary.csv")
    v311_summary = read_csv("paper4_v311_post_v310_one_swap_summary.csv")
    v47_panel = read_parquet("paper4_v47_ifrs9_proxy_panel_v45.parquet")
    if any(
        df.empty
        for df in [universe, selected, source_summary, v312_summary, v311_summary, v47_panel]
    ):
    ...
    policy_id = "v312_post_v310_best_one_swap_repair_candidate"
    regime = "post_v312_repair_candidate"
    v312_row = v312_summary.iloc[0]
    v311_row = v311_summary.iloc[0]
    current_exposure = float(v312_row[f"portfolio_exposure_v{BASE_REPAIR_VERSION}"])
    exposure_min = float(v311_row[f"exposure_min_v{PREVIOUS_REPRICE_VERSION}"])
    exposure_max = float(v311_row[f"exposure_max_v{PREVIOUS_REPRICE_VERSION}"])
    cvar_cap = float(v312_row[f"scenario_loss_cvar90_v{BASE_REPAIR_VERSION}"])
    current_objective_return = float(v312_row[f"objective_return_v{BASE_REPAIR_VERSION}"])
    ...
    for candidate_pos, selected_pos in np.argwhere(source_prefilter_mask):
        ...
        row: dict[str, Any] = {
            ...
            f"claim_boundary_v{VERSION}": (
                "post-v312 one-swap pricing only; not multi-swap or global proof"
            ),
    ...
    local_claim_boundary = (
        "post-v312 one-swap screen cleared; dynamic/global gates still missing"
        if improving_pairs == 0
        else "post-v312 one-swap screen found improvements; repair/reprice loop must continue"
    )
        ...
        [
            {
                ...
                f"post_v312_one_swap_local_optimality_cleared_v{VERSION}": (improving_pairs == 0),
    ...
    stage_summary[f"claim_boundary_v{VERSION}"] = (
        "post-v312 repair one-swap screen stage count only"
    )
...
def _claim_blockers(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_apply_next_post_v312_swap.csv"
        if improving > 0
        else f"paper4_v{NEXT_VERSION}_dynamic_proxy_or_global_bound_after_v312.csv"
    )
    return pd.DataFrame(
        [
            {
                f"blocker_id_v{VERSION}": "post_v312_one_swap_improvement_found",
                f"blocking_v{VERSION}": improving > 0,
                f"evidence_count_v{VERSION}": improving,
                f"required_next_artifact_v{VERSION}": next_artifact,
                f"claim_boundary_v{VERSION}": (
                    "feasible improving post-v312 one-swaps block local optimality"
                    if improving > 0
                    else "no feasible improving post-v312 one-swaps remain"
                ),
            },
            {
                f"blocker_id_v{VERSION}": "post_v312_repair_not_working_champion",
            ...
            },
            {
                ...
                f"claim_boundary_v{VERSION}": "v312 portfolio still has missing v47 proxy rows",
            },
            {
...
def _claim_matrix(*, local_optimality_cleared: bool) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v313_post_v312_one_swap_reprice_executed",
                "allowed": True,
                "artifact": "paper4_v313_post_v312_one_swap_summary.csv",
                "boundary": "post-v312 one-drop/one-add screen completed",
            },
            {
                "claim_id": "v313_post_v312_one_swap_local_optimality",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v313_claim_blockers.csv",
                "boundary": "allowed only within one-drop/one-add scope",
            },
            {
                "claim_id": "v313_dynamic_gate_ready",
                "allowed": local_optimality_cleared,
                "artifact": "paper4_v313_post_v312_one_swap_summary.csv",
                "boundary": "ready only to attempt proxy dynamic/global gate, not to promote",
            },
            {
                "claim_id": "v313_working_champion",
                "allowed": False,
                "artifact": "paper4_v313_claim_blockers.csv",
                "boundary": "global, dynamic, online and deployment evidence missing",
            },
            {
                "claim_id": "v313_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v313_claim_blockers.csv",
...
def _update_claim_boundaries(*, local_optimality_cleared: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v313 post-v312 one-swap repricing gate.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v313_post_v312_one_swap_summary.csv"
                ),
                "boundary": "One-drop/one-add screen after the v312 repaired candidate.",
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v313 clears post-v312 one-swap local optimality.",
                "allowed": local_optimality_cleared,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v313_claim_blockers.csv"
                ),
                "boundary": "Scope-limited to one-drop/one-add swaps after v312.",
                "prohibited_claim_flag": not local_optimality_cleared,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v313 authorizes a Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v313_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v313 proves full-universe global integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v313_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v313 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v313_claim_blockers.csv"
                ),
...
def _update_backlog(*, local_optimality_cleared: bool) -> None:
    ...
    next_artifact = (
        f"paper4_v{NEXT_VERSION}_dynamic_proxy_or_global_bound_after_v312.csv"
        if local_optimality_cleared
        else f"paper4_v{NEXT_VERSION}_apply_next_post_v312_swap.csv"
    )
    additions = pd.DataFrame(
        [
            {
                ...
                "executable_item": (
                    "v313 reprices the v312 repaired candidate against all non-selected "
                    "comparable loans with exact budget/source/CVaR screens."
                ),
                "status": (
                    "post_v312_one_swap_local_optimality_cleared"
                    if local_optimality_cleared
                    else "post_v312_one_swap_improvement_found"
                ),
                "next_artifact": next_artifact,
                "success_condition": (
                    "if cleared, run a dynamic/global gate; if not, apply the next "
                    "post-v312 repair and reprice again"
                ),
                "last_wave": "v313",
                "execution_result": (
                    "post_v312_one_swap_reprice_cleared"
                    if local_optimality_cleared
                    else "post_v312_one_swap_reprice_found_improvements"
                ),
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v313")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V313_POST_V312_ONE_SWAP_REPRICE_START -->"
    end = "<!-- V313_POST_V312_ONE_SWAP_REPRICE_END -->"
    best_delta = status["best_one_swap_return_delta_v313"]
    best_delta_text = (
        "not applicable; no source-exact return-improving swaps"
        if best_delta is None
        else str(best_delta)
    )
    best_feasible = status["best_feasible_one_swap_return_delta_v313"]
    best_feasible_text = (
...
- Selected rows: `{status["selected_rows_v313"]}`.
- Candidate add rows: `{status["candidate_add_rows_v313"]}`.
- Pair rows screened: `{status["total_pair_rows_screened_v313"]}`.
- Return-improving pairs: `{status["return_improving_pair_rows_v313"]}`.
- Budget+return feasible pairs: `{status["budget_return_feasible_pair_rows_v313"]}`.
- Source prefilter pairs: `{status["source_prefilter_pair_rows_v313"]}`.
- Exact source-feasible pairs: `{status["source_exact_pair_rows_v313"]}`.
- CVaR-feasible improving one-swaps: `{status["one_swap_improving_rows_v313"]}`.
...
- Current missing v47 proxy rows:
  `{status["current_missing_v47_proxy_rows_v313"]}`.
- Best feasible delta missing proxy rows:
  `{status["best_feasible_delta_missing_v47_proxy_rows_v313"]}`.
- Post-v312 one-swap local optimality cleared:
  `{status["post_v312_one_swap_local_optimality_cleared_v313"]}`.
- Dynamic/global gate ready:
  `{status["dynamic_gate_ready_v313"]}`.
...
def main() -> None:
    ...
    local_optimality_cleared = bool(
        summary[f"post_v312_one_swap_local_optimality_cleared_v{VERSION}"].iloc[0]
    )
    blockers = _claim_blockers(summary)
    claim_matrix = _claim_matrix(local_optimality_cleared=local_optimality_cleared)

    write_csv(TABLE_DIR / "paper4_v313_post_v312_one_swap_reprice.csv", pairs)
    write_csv(TABLE_DIR / "paper4_v313_post_v312_one_swap_top_candidates.csv", top_candidates)
    write_csv(TABLE_DIR / "paper4_v313_post_v312_one_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v313_post_v312_one_swap_stage_summary.csv", stage_summary)
    write_csv(TABLE_DIR / "paper4_v313_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v313_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v313_post_v312_reprice",
        "schema_version": "2026-05-16.313",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v313": BASE_REPAIR_VERSION,
        "previous_reprice_version_v313": PREVIOUS_REPRICE_VERSION,
        "summary_rows_v313": int(len(summary)),
        "stage_summary_rows_v313": int(len(stage_summary)),
        "candidate_pair_rows_v313": int(len(pairs)),
        "top_candidate_rows_v313": int(len(top_candidates)),
        "claim_blocker_rows_v313": int(len(blockers)),
        "claim_matrix_rows_v313": int(len(claim_matrix)),
        "selected_rows_v313": int(row[f"selected_rows_v{VERSION}"]),
        "base_selected_rows_v313": int(row[f"base_selected_rows_v{VERSION}"]),
        "cardinality_restored_v313": bool(row[f"cardinality_restored_v{VERSION}"]),
        "candidate_add_rows_v313": int(row[f"candidate_add_rows_v{VERSION}"]),
        "total_pair_rows_screened_v313": int(row[f"total_pair_rows_screened_v{VERSION}"]),
        "return_improving_pair_rows_v313": int(row[f"return_improving_pair_rows_v{VERSION}"]),
        "budget_return_feasible_pair_rows_v313": int(
            row[f"budget_return_feasible_pair_rows_v{VERSION}"]
        ),
        "source_prefilter_pair_rows_v313": int(row[f"source_prefilter_pair_rows_v{VERSION}"]),
        "source_exact_pair_rows_v313": int(row[f"source_exact_pair_rows_v{VERSION}"]),
        "cvar_feasible_pair_rows_v313": int(row[f"cvar_feasible_pair_rows_v{VERSION}"]),
        "one_swap_improving_rows_v313": int(row[f"one_swap_improving_rows_v{VERSION}"]),
        "best_one_swap_return_delta_v313": _float_or_none(best_delta),
        "best_one_swap_cvar90_after_v313": _float_or_none(best_cvar),
        "best_feasible_one_swap_return_delta_v313": _float_or_none(best_feasible_delta),
        "best_feasible_one_swap_cvar90_after_v313": _float_or_none(best_feasible_cvar),
        "current_missing_v47_proxy_rows_v313": int(
            row[f"current_missing_v47_proxy_rows_v{VERSION}"]
        ),
        "best_feasible_delta_missing_v47_proxy_rows_v313": int(
            row[f"best_feasible_delta_missing_v47_proxy_rows_v{VERSION}"]
        ),
        "current_exposure_v313": float(row[f"current_exposure_v{VERSION}"]),
        "current_objective_return_v313": float(row[f"current_objective_return_v{VERSION}"]),
        "current_loss_mean_v313": float(row[f"current_loss_mean_v{VERSION}"]),
        "current_cvar90_v313": float(row[f"current_cvar90_v{VERSION}"]),
        "post_v312_one_swap_local_optimality_cleared_v313": local_optimality_cleared,
        "dynamic_gate_ready_v313": local_optimality_cleared,
        "working_champion_claim_allowed_v313": False,
        "full_universe_integer_optimality_claim_allowed_v313": False,
        "paper1_promotion_allowed_v313": False,
        "paper4_working_champion_changed_v313": False,
        ...
        "claim_boundary": (
            "v313 is a post-v312 one-swap repricing gate; no working champion or "
            "final promotion is authorized"
        ),
    }
    write_json(STATUS_DIR / "paper4_v313_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v313": status}, indent=2, sort_keys=True))
