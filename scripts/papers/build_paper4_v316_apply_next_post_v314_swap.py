#!/usr/bin/env python3
"""Build Paper 4 v316 applied post-v314 one-swap repair artifacts."""
...
def _best_signal() -> pd.Series:
    top = read_csv("paper4_v315_post_v314_one_swap_top_candidates.csv")
    if top.empty:
        raise RuntimeError("Missing v315 top candidates; run v315 first.")
    feasible = top.loc[top[f"one_swap_improves_return_v{SIGNAL_VERSION}"].astype(bool)].copy()
    if feasible.empty:
        raise RuntimeError("v316 requires a feasible improving v315 one-swap signal.")
    ...
    *,
    ...
    for family in FAMILIES:
        ...
        for source_id in sorted(universe[family].dropna().astype(str).unique()):
            ...
            rows.append(
                {
                    f"portfolio_label_v{VERSION}": "post_v314_best_one_swap_repair_candidate",
                    ...
                    f"claim_boundary_v{VERSION}": "v316 post-swap source diagnostic only",
                }
            )
...
def _build_repair() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base_summary = read_csv("paper4_v314_apply_next_post_v312_swap_summary.csv")
    signal_summary = read_csv("paper4_v315_post_v314_one_swap_summary.csv")
    v47_panel = read_parquet("paper4_v47_ifrs9_proxy_panel_v45.parquet")
    source_caps = read_csv("paper4_v314_apply_next_post_v312_swap_source_summary.csv")
    best = _best_signal()
    if any(
        df.empty
        for df in [universe, selected, base_summary, signal_summary, v47_panel, source_caps]
    ):
        raise RuntimeError("Missing v55, v314, v315 or v47 inputs for v316.")
    ...
    if len(kept) != len(selected) - 1:
        raise RuntimeError(f"Could not drop selected loan {drop_id} from v314 portfolio.")
    ...
    repaired[f"portfolio_label_v{VERSION}"] = "post_v314_best_one_swap_repair_candidate"
    ...
    repaired[f"claim_boundary_v{VERSION}"] = (
        "post-v314 one-swap repair candidate only; requires post-repair repricing"
    )
        ...
        [
            {
                ...
                f"dropped_had_observed_proxy_v{VERSION}": drop_id in observed_ids,
                f"claim_boundary_v{VERSION}": (
                    "best v315 feasible swap applied; not post-repair optimality"
                ),
            }
        ]
    )
        ...
        [
            {
                ...
                f"next_artifact_v{VERSION}": f"paper4_v{NEXT_VERSION}_post_v316_reprice.csv",
                f"claim_boundary_v{VERSION}": (
                    "applied best v315 one-swap repair; repricing and proxy coverage gates remain"
                ),
            }
        ]
    )
...
def _claim_blockers(summary: pd.DataFrame, coverage: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {
                f"blocker_id_v{VERSION}": "best_v315_swap_repair_candidate_created",
                f"blocking_v{VERSION}": not feasible,
                f"evidence_count_v{VERSION}": int(feasible),
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v316_apply_next_post_v314_swap_summary.csv"
                ),
                f"claim_boundary_v{VERSION}": (
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": f"paper4_v{NEXT_VERSION}_post_v316_reprice.csv",
...
def _claim_matrix() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "claim_id": "v316_best_v315_swap_applied",
                "allowed": True,
                "artifact": "paper4_v316_apply_next_post_v314_swap_summary.csv",
                "boundary": "single feasible v315 repair applied and audited",
            },
            {
                "claim_id": "v316_return_improves_and_cvar_lowers_vs_v314",
                "allowed": True,
                "artifact": "paper4_v316_apply_next_post_v314_swap_summary.csv",
                "boundary": "static scenario proxy metrics only; requires repricing",
            },
            {
                "claim_id": "v316_post_repair_local_optimality",
                "allowed": False,
                "artifact": "paper4_v316_claim_blockers.csv",
                "boundary": "post-v316 repricing has not been executed",
            },
            {
                "claim_id": "v316_working_champion",
                "allowed": False,
                "artifact": "paper4_v316_claim_blockers.csv",
                "boundary": "global, dynamic, online and deployment evidence missing",
            },
            {
                "claim_id": "v316_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v316_claim_blockers.csv",
...
def _update_claim_boundaries() -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v316 applied post-v314 one-swap repair candidate.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v316_apply_next_post_v314_swap_summary.csv"
                ),
                "boundary": "Best feasible v315 swap applied; requires post-repair repricing.",
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v316 improves return and lowers CVaR versus v314.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/"
                    "paper4_v316_apply_next_post_v314_swap_summary.csv"
            ...
            {
                "claim": "v316 repaired portfolio is post-repair locally optimal.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v316_claim_blockers.csv"
                ),
                "boundary": "Post-v316 repricing has not been executed.",
            ...
            {
                "claim": "v316 authorizes a Paper 4 working champion.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v316_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v316 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v316_claim_blockers.csv"
                ),
...
def _update_backlog() -> None:
        ...
        [
            {
                ...
                "executable_item": (
                    "v316 applies the best CVaR-feasible v315 one-swap signal to the "
                    "v314 repaired candidate and audits return, CVaR, source caps and "
                    "proxy coverage."
                ),
                "status": "post_v314_best_one_swap_applied_requires_repricing",
                "next_artifact": f"paper4_v{NEXT_VERSION}_post_v316_reprice.csv",
                "success_condition": (
                    "post-v316 repricing finds no feasible improving one-swaps or records "
    ...
    if current.empty:
        write_csv(path, additions)
        return
    out = current.loc[~current["last_wave"].astype(str).eq("v316")].copy()
    write_csv(path, pd.concat([out, additions], ignore_index=True))


def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V316_APPLY_POST_V314_SWAP_START -->"
    end = "<!-- V316_APPLY_POST_V314_SWAP_END -->"
    block = f"""
{start}
...
- Added loan: `{status["added_loan_id_v316"]}`.
- Dropped loan: `{status["dropped_loan_id_v316"]}`.
- Return delta vs v314: `{status["delta_return_vs_v314_v316"]}`.
- CVaR90 delta vs v314: `{status["delta_cvar90_vs_v314_v316"]}`.
- Objective return: `{status["objective_return_v316"]}`.
- CVaR90 after repair: `{status["scenario_loss_cvar90_v316"]}`.
- Source cap violations: `{status["source_cap_violations_v316"]}`.
- Missing v47 proxy rows: `{status["missing_v47_proxy_rows_v316"]}`.
- Delta missing proxy rows vs v314:
  `{status["delta_missing_v47_proxy_rows_vs_v314_v316"]}`.
- Repair candidate feasible: `{status["repair_candidate_feasible_v316"]}`.
- Post-repair repricing required:
  `{status["post_repair_pricing_required_v316"]}`.
...
def main() -> None:
    ...
    allocations.to_parquet(TABLE_DIR / "paper4_v316_apply_next_post_v314_swap_allocations.parquet")
    write_csv(TABLE_DIR / "paper4_v316_apply_next_post_v314_swap_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v316_apply_next_post_v314_swap_action.csv", action)
    write_csv(
        TABLE_DIR / "paper4_v316_apply_next_post_v314_swap_source_summary.csv", source_summary
    )
    write_csv(TABLE_DIR / "paper4_v316_proxy_coverage_delta.csv", coverage)
    write_csv(TABLE_DIR / "paper4_v316_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v316_claim_matrix_delta.csv", claim_matrix)
    ...
    status = {
        "phase": "v316_apply_next_post_v314_swap",
        "schema_version": "2026-05-16.316",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "signal_version_v316": SIGNAL_VERSION,
        "base_version_v316": BASE_VERSION,
        "baseline_version_v316": BASELINE_VERSION,
        "summary_rows_v316": int(len(summary)),
        "action_rows_v316": int(len(action)),
        "allocation_rows_v316": int(len(allocations)),
        "source_summary_rows_v316": int(len(source_summary)),
        "coverage_rows_v316": int(len(coverage)),
        "claim_blocker_rows_v316": int(len(blockers)),
        "claim_matrix_rows_v316": int(len(claim_matrix)),
        "added_loan_id_v316": str(action_row[f"added_loan_id_v{VERSION}"]),
        "dropped_loan_id_v316": str(action_row[f"dropped_loan_id_v{VERSION}"]),
        "added_has_observed_proxy_v316": bool(action_row[f"added_has_observed_proxy_v{VERSION}"]),
        "dropped_had_observed_proxy_v316": bool(
            action_row[f"dropped_had_observed_proxy_v{VERSION}"]
        ),
        "selected_rows_v316": int(row[f"selected_rows_v{VERSION}"]),
        "base_selected_rows_v316": int(row[f"base_selected_rows_v{VERSION}"]),
        "cardinality_restored_v316": bool(row[f"cardinality_restored_v{VERSION}"]),
        "portfolio_exposure_v316": float(row[f"portfolio_exposure_v{VERSION}"]),
        "objective_return_v316": float(row[f"objective_return_v{VERSION}"]),
        "scenario_loss_mean_v316": float(row[f"scenario_loss_mean_v{VERSION}"]),
        "scenario_loss_cvar90_v316": float(row[f"scenario_loss_cvar90_v{VERSION}"]),
        "source_cap_violations_v316": int(row[f"source_cap_violations_v{VERSION}"]),
        "max_source_share_v316": float(row[f"max_source_share_v{VERSION}"]),
        "min_source_slack_v316": float(row[f"min_source_slack_v{VERSION}"]),
        "observed_v47_proxy_rows_v316": int(row[f"observed_v47_proxy_rows_v{VERSION}"]),
        "missing_v47_proxy_rows_v316": int(row[f"missing_v47_proxy_rows_v{VERSION}"]),
        "delta_missing_v47_proxy_rows_vs_v314_v316": int(
            row[f"delta_missing_v47_proxy_rows_vs_v{BASE_VERSION}_v{VERSION}"]
        ),
        "delta_return_vs_v314_v316": float(row[f"delta_return_vs_v{BASE_VERSION}_v{VERSION}"]),
        "delta_cvar90_vs_v314_v316": float(row[f"delta_cvar90_vs_v{BASE_VERSION}_v{VERSION}"]),
        "delta_exposure_vs_v314_v316": float(row[f"delta_exposure_vs_v{BASE_VERSION}_v{VERSION}"]),
        "budget_feasible_v316": bool(row[f"budget_feasible_v{VERSION}"]),
        "cvar_feasible_v316": bool(row[f"cvar_feasible_v{VERSION}"]),
        "source_feasible_v316": bool(row[f"source_feasible_v{VERSION}"]),
        "repair_candidate_feasible_v316": bool(row[f"repair_candidate_feasible_v{VERSION}"]),
        "post_repair_pricing_required_v316": bool(row[f"post_repair_pricing_required_v{VERSION}"]),
        "post_repair_one_swap_optimality_claim_allowed_v316": False,
        "working_champion_claim_allowed_v316": False,
        "full_universe_integer_optimality_claim_allowed_v316": False,
        "paper1_promotion_allowed_v316": False,
        "paper4_working_champion_changed_v316": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "next_artifact_v316": f"paper4_v{NEXT_VERSION}_post_v316_reprice.csv",
        "claim_boundary": (
            "v316 applies a post-v314 one-swap repair; no working champion or final "
            "promotion is authorized"
        ),
    }
    write_json(STATUS_DIR / "paper4_v316_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v316": status}, indent=2, sort_keys=True))
