#!/usr/bin/env python3
"""Build Paper 4 v272 expanded restricted-pool MILP/gap probe artifacts."""
...
VERSION = 272
BASE_REPAIR_VERSION = 270
ONE_SWAP_REPRICE_VERSION = 271
...
def _build_pool(
    ...
    omitted = omitted.assign(mean_return_v272=mean_returns[omitted.index.to_numpy()])
    ...
    top_ids = set(
        omitted.sort_values("mean_return_v272", ascending=False)
        .head(CANDIDATE_POOL_LIMIT)["loan_id"]
        .astype(str)
    )
    candidate_ids = top_ids | frontier_add_ids

    selected_pool = selected.copy()
    selected_pool["pool_role_v272"] = "current_v270_selected"
    candidate_pool = omitted.loc[omitted["loan_id"].astype(str).isin(candidate_ids)].copy()
    candidate_pool["pool_role_v272"] = np.where(
        candidate_pool["loan_id"].astype(str).isin(frontier_add_ids),
        "omitted_candidate_from_top1500_or_v268_frontier",
        "omitted_candidate_from_top1500_mean_return",
    )
    keep_cols = ["loan_id", "loan_amnt", *FAMILIES, "pool_role_v272"]
...
def _update_claim_boundaries(*, improvement_found: bool, incumbent_available: bool) -> None:
    ...
    additions = pd.DataFrame(
        [
            {
                "claim": "Paper 4 has a v272 restricted-pool MILP/gap probe after v271.",
                "allowed": True,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v272_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Restricted to current v270 selected loans plus a top-1500 omitted candidate "
                    "pool and v268 frontier loans; not full universe."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v272 finds a better restricted-pool MILP incumbent than v270.",
                "allowed": bool(improvement_found and incumbent_available),
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v272_restricted_pool_milp_summary.csv"
                ),
                "boundary": (
                    "Existence claim only inside the restricted v272 pool and solver tolerance."
                ),
                "prohibited_claim_flag": False,
                "current_quarto_page": "living_notebook_only",
            },
            {
                "claim": "v272 proves global full-universe integer optimality.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v272_claim_blockers.csv"
                ),
            ...
            {
                "claim": "v272 replaces Paper Estrella or finalizes Paper 4.",
                "allowed": False,
                "evidence_artifact": (
                    "reports/paper_material/paper4/tables/paper4_v272_claim_blockers.csv"
                ),
        ...
        [
            {
                ...
                "lane": "CVaR/OCE",
                "executable_item": (
                    "v272 solves an expanded restricted-pool binary MILP over the v270 selected "
                    "loans plus top omitted candidates after v271 cleared one-swap local pricing."
                ),
                "status": (
                ...
                "next_artifact": (
                    "paper4_v273_apply_restricted_pool_candidate_or_decompose_swaps.csv"
                    if improvement_found
                    else "paper4_v273_full_universe_gap_certificate_protocol.csv"
                ),
                "success_condition": (
...
def _update_notebook(status: dict[str, Any]) -> None:
    start = "<!-- V272_RESTRICTED_POOL_MILP_GAP_PROBE_START -->"
    end = "<!-- V272_RESTRICTED_POOL_MILP_GAP_PROBE_END -->"
    block = f"""
{start}
...
- Pool rows: `{status["pool_rows_v272"]}`.
- Current selected rows: `{status["current_selected_rows_v272"]}`.
- Omitted candidate rows: `{status["omitted_candidate_rows_v272"]}`.
- MILP incumbent available: `{status["milp_incumbent_available_v272"]}`.
- MILP success flag: `{status["milp_success_v272"]}`.
- MILP gap: `{status["milp_gap_v272"]}`.
- Objective delta vs v270:
  `{status["objective_delta_vs_v270_v272"]}`.
- CVaR90 delta vs v270:
  `{status["cvar90_delta_vs_v270_v272"]}`.
- Better restricted-pool incumbent found:
  `{status["restricted_pool_improvement_found_v272"]}`.
...
def main() -> None:
    ...
    if universe.empty or selected.empty or source_caps.empty:
        raise RuntimeError("Missing v55 universe, v270 selected portfolio, or source caps.")
    ...
    pool[f"milp_action_v{VERSION}"] = np.select(
        ...
        ["dropped_by_restricted_pool_milp", "added_by_restricted_pool_milp", "kept_from_v270"],
    ...
    summary = pd.DataFrame(
        [
            {
                f"probe_label_v{VERSION}": "restricted_pool_milp_after_v271_no_two_swap_improvement",
                ...
                f"objective_delta_vs_v270_v{VERSION}": objective_delta,
                f"portfolio_exposure_v{VERSION}": exposure,
                f"scenario_loss_cvar90_v{VERSION}": cvar90,
                f"cvar90_delta_vs_v270_v{VERSION}": cvar_delta,
    ...
    blockers = pd.DataFrame(
        [
            ...
            },
            {
                ...
                f"required_next_artifact_v{VERSION}": (
                    "paper4_v273_full_universe_gap_certificate_protocol.csv"
                ),
    ...
    claim_matrix = pd.DataFrame(
        [
            {
                "claim_id": "v272_restricted_pool_milp_probe_executed",
                "allowed": True,
                "artifact": "paper4_v272_restricted_pool_milp_summary.csv",
                "boundary": "restricted pool only",
            },
            {
                "claim_id": "v272_restricted_pool_improvement_found",
                "allowed": restricted_improvement,
                "artifact": "paper4_v272_restricted_pool_milp_summary.csv",
                "boundary": "inside v272 restricted pool and solver result only",
            },
            {
                "claim_id": "v272_global_full_universe_integer_optimality",
                "allowed": False,
                "artifact": "paper4_v272_claim_blockers.csv",
                "boundary": "full-universe gap certificate missing",
            },
            {
                "claim_id": "v272_paper1_or_final_promotion",
                "allowed": False,
                "artifact": "paper4_v272_claim_blockers.csv",
    ...
    pool.to_parquet(
        TABLE_DIR / "paper4_v272_restricted_pool_milp_allocations.parquet",
        index=False,
        compression="zstd",
    )
    write_csv(TABLE_DIR / "paper4_v272_restricted_pool_milp_summary.csv", summary)
    write_csv(TABLE_DIR / "paper4_v272_restricted_pool_milp_source_summary.csv", source_diag)
    write_csv(TABLE_DIR / "paper4_v272_restricted_pool_milp_constraints.csv", constraints)
    write_csv(TABLE_DIR / "paper4_v272_claim_blockers.csv", blockers)
    write_csv(TABLE_DIR / "paper4_v272_claim_matrix_delta.csv", claim_matrix)
    ...
    row = summary.iloc[0]
    status = {
        "phase": "v272_restricted_pool_milp_gap_probe_after_v271",
        "schema_version": "2026-05-15.272",
        "generated_at_utc": now(),
        "runtime_seconds": round((datetime.now(UTC) - started).total_seconds(), 3),
        "base_repair_version_v272": BASE_REPAIR_VERSION,
        "terminal_reprice_version_v272": ONE_SWAP_REPRICE_VERSION,
        "bounded_two_swap_version_v272": BOUNDED_TWO_SWAP_VERSION,
        "candidate_pool_limit_v272": CANDIDATE_POOL_LIMIT,
        "pool_rows_v272": int(row["pool_rows_v272"]),
        "current_selected_rows_v272": int(row["current_selected_rows_v272"]),
        "omitted_candidate_rows_v272": int(row["omitted_candidate_rows_v272"]),
        "selected_rows_v272": int(row["selected_rows_v272"]),
        "added_rows_v272": int(row["added_rows_v272"]),
        "dropped_rows_v272": int(row["dropped_rows_v272"]),
        "objective_return_v272": float(row["objective_return_v272"]),
        "objective_delta_vs_v270_v272": float(row["objective_delta_vs_v270_v272"]),
        "portfolio_exposure_v272": float(row["portfolio_exposure_v272"]),
        "scenario_loss_cvar90_v272": float(row["scenario_loss_cvar90_v272"]),
        "cvar90_delta_vs_v270_v272": float(row["cvar90_delta_vs_v270_v272"]),
        "source_cap_violations_v272": int(row["source_cap_violations_v272"]),
        "budget_feasible_v272": bool(row["budget_feasible_v272"]),
        "cvar_feasible_v272": bool(row["cvar_feasible_v272"]),
        "source_feasible_v272": bool(row["source_feasible_v272"]),
        "restricted_pool_improvement_found_v272": restricted_improvement,
        "milp_success_v272": bool(row["milp_success_v272"]),
        "milp_incumbent_available_v272": bool(row["milp_incumbent_available_v272"]),
        "milp_status_v272": int(row["milp_status_v272"]),
        "milp_gap_v272": float(row["milp_gap_v272"]),
        "milp_dual_bound_v272": float(row["milp_dual_bound_v272"]),
        "constraint_rows_v272": int(row["constraint_rows_v272"]),
        "claim_blocker_rows_v272": int(len(blockers)),
        "claim_matrix_rows_v272": int(len(claim_matrix)),
        "restricted_pool_global_optimality_claim_allowed_v272": False,
        "full_universe_integer_optimality_claim_allowed_v272": False,
        "paper1_promotion_allowed_v272": False,
        "paper4_working_champion_changed_v272": False,
        "paper4_final_promotion_created": FORBIDDEN_FINAL_PROMOTION.exists(),
        "claim_boundary": (
            "v272 is a restricted-pool MILP/gap probe only; full-universe global "
            "and promotion claims remain blocked"
        ),
    }
    write_json(STATUS_DIR / "paper4_v272_status.json", status)
    _update_notebook(status)
    print(json.dumps({"v272": status}, indent=2, sort_keys=True))
