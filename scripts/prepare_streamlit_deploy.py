...
DATASET_SHAPE_ASSETS: list[tuple[str, str]] = [
    ...
    ("data/processed/time_series_full.parquet", "parquet"),
    ("data/processed/time_series_panel.parquet", "parquet"),
...
REQUIRED_FILES = [
    ...
    "data/processed/cate_portfolio_comparison.parquet",
    ...
    "data/processed/ab_simulation_results.parquet",
    "data/processed/ab_simulation_summary.parquet",
    "data/processed/ale_curves.parquet",
    "data/processed/explainability_global.parquet",
    "data/processed/explainability_local_cases.parquet",
    "data/processed/explanation_drift.parquet",
    "data/processed/fairness_audit.parquet",
...
OPTIONAL_FILES = [
    ...
    "data/processed/drift_checks.parquet",
    "data/processed/pdp_ice_top5.parquet",
...
def _sync_streamlit_requirements(output_dir: Path) -> None:
    """Create Streamlit Cloud-recognized dependency files from source list."""
    source = output_dir / "requirements.streamlit.txt"
    if not source.exists():
        return
    content = source.read_text(encoding="utf-8").strip() + "\n"
    (output_dir / "requirements.txt").write_text(content, encoding="utf-8")
    app_requirements = output_dir / "streamlit_app" / "requirements.txt"
    app_requirements.parent.mkdir(parents=True, exist_ok=True)
    app_requirements.write_text(content, encoding="utf-8")
...
def build_bundle(output_dir: Path, clean: bool, strict: bool, skip_duckdb: bool) -> None:
    ...
    _sync_streamlit_requirements(output_dir)
